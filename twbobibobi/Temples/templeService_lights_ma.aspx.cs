...
{
    /// <summary>
    /// 玉敕大樹朝天宮 點燈服務頁面 (光明燈/太歲燈/五文昌燈/福財燈)
    /// </summary>
    /// <remarks>
    /// 此頁面繼承自 AjaxBasePage，負責：
    /// 1. 控制祈福燈顯示狀態（光明燈、太歲燈、五文昌燈、福財燈）
    /// 2. 初始化 Ajax 處理器（建立/修改報名資料）
    /// 3. 處理購買人與祈福人資料寫入流程
    /// </remarks>
    public partial class templeService_lights_ma : AjaxBasePage
    {
        ...
        public class AjaxPageHandler
        {
            ...
            public void gotochecked(BasePage basePage)
            {
                ...
                string postURL_Init = "Lights_ma_Index";
                ...
                for (int i = 0; i < listcount; i++)
                {
                    ...
                    {
                        case "光明燈": count_ma_lights[0]++; break;
                        case "太歲燈": count_ma_lights[1]++; break;
                        case "五文昌燈": count_ma_lights[2]++; break;
                        case "福財燈": count_ma_lights[3]++; break;
                    }
                }
                ...
                for (int i = 0; i < JLightsString_Tag.Count; i++)
                {
                    ...
                    {
                        checkednum_ma = false;
                        basePage.mJSonHelper.AddContent("overnumType", GetLightsType(JLightsString_Tag[i].ToString(), "23"));
                ...
                if (checkednum_ma)
                {
                    ...
                    // === 建立購買人資料 ===
                    ApplicantID = objLightDAC.Addapplicantinfo_lights_ma(
                        Name: AppName,
                        Mobile: AppMobile,
                        Cost: "0",
                        County: Appcounty,
                    ...
                    if (ApplicantID > 0)
                    {
                        for (int i = 0; i < listcount; i++)
                        {
                            ...
                            string lightsType = GetLightsType(lightsString, "23");
                            ...
                            {
                                lightsinfo = true;
                                LightsID = objLightDAC.AddLights_ma(
