...
{
    /// <summary>
    /// 壽山巖觀音寺點燈服務頁面 (平安燈(光明燈)/安太歲/文昌燈/招財燈/姻緣燈/貴人燈/元辰燈/添丁燈/婚姻燈/安太陰/安太陽)
    /// </summary>
    /// <remarks>
    /// 此頁面繼承自 AjaxBasePage，負責：
    /// 1. 控制祈福燈顯示狀態（平安燈(光明燈)、安太歲、文昌燈、招財燈、姻緣燈、貴人燈、元辰燈、添丁燈、婚姻燈、安太陰、安太陽）
    /// 2. 初始化 Ajax 處理器（建立/修改報名資料）
    /// 3. 處理購買人與祈福人資料寫入流程
    /// </remarks>
    public partial class templeService_lights_ssy : AjaxBasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                //this.light9.Visible = false;
                //this.light10.Visible = false;
                //this.light11.Visible = false;
                ...
                //// 顯示姻緣燈
                //this.light5.Visible = objLightDAC.CheckedLightsNum("7", adminID.ToString(), 1, Year, this);

                //// 顯示貴人燈
                //this.light6.Visible = objLightDAC.CheckedLightsNum("10", adminID.ToString(), 1, Year, this);

                //// 顯示元辰燈
                //this.light7.Visible = objLightDAC.CheckedLightsNum("43", adminID.ToString(), 1, Year, this);

                //// 顯示添丁燈
                //this.light8.Visible = objLightDAC.CheckedLightsNum("46", adminID.ToString(), 1, Year, this);

                //// 顯示婚姻燈
                //this.light9.Visible = objLightDAC.CheckedLightsNum("47", adminID.ToString(), 1, Year, this);

                //// 顯示安太陰
                //this.light10.Visible = objLightDAC.CheckedLightsNum("48", adminID.ToString(), 1, Year, this);

                //// 顯示安太陽
                //this.light11.Visible = objLightDAC.CheckedLightsNum("49", adminID.ToString(), 1, Year, this);
        ...
        public class AjaxPageHandler
        {
            ...
            public void gotochecked(BasePage basePage)
            {
                ...
                string postURL_Init = "Lights_ssy_Index";
                string url = HttpContext.Current.Request.Url.AbsoluteUri;
                string postURL = GetRequestURL(url, postURL_Init);

                // === 確認各燈種數量 ===
                int[] count_ssy_lights = new int[11];
                bool checkednum_ssy = true;
                for (int i = 0; i < listcount; i++)
                {
                    switch (JLightsString_Tag[i].ToString())
                    {
                        case "光明燈": count_ssy_lights[0]++; break;
                        case "安太歲": count_ssy_lights[1]++; break;
                        case "文昌燈": count_ssy_lights[2]++; break;
                        case "招財燈": count_ssy_lights[3]++; break;
                        case "姻緣燈": count_ssy_lights[4]++; break;
                        case "貴人燈": count_ssy_lights[5]++; break;
                        case "元辰燈": count_ssy_lights[6]++; break;
                        case "添丁燈": count_ssy_lights[7]++; break;
                        case "婚姻燈": count_ssy_lights[8]++; break;
                        case "安太陰": count_ssy_lights[9]++; break;
                        case "安太陽": count_ssy_lights[10]++; break;
                    }
                }
                ...
                {
                    string lightsType = GetLightsType(JLightsString_Tag[i].ToString(), "41");
                    int c = 0;

                    c += lightsType == "3" ? count_ssy_lights[0] : 0;
                    c += lightsType == "4" ? count_ssy_lights[1] : 0;
                    c += lightsType == "5" ? count_ssy_lights[2] : 0;
                    c += lightsType == "6" ? count_ssy_lights[3] : 0;
                    c += lightsType == "7" ? count_ssy_lights[4] : 0;
                    c += lightsType == "10" ? count_ssy_lights[5] : 0;
                    c += lightsType == "43" ? count_ssy_lights[6] : 0;
                    c += lightsType == "46" ? count_ssy_lights[7] : 0;
                    c += lightsType == "47" ? count_ssy_lights[8] : 0;
                    c += lightsType == "48" ? count_ssy_lights[9] : 0;
                    c += lightsType == "49" ? count_ssy_lights[10] : 0;

                    if (objLightDAC.CheckedLightsNum(lightsType, AdminID.ToString(), c, Year, basePage))
                    {
                        checkednum_ssy = false;
                        basePage.mJSonHelper.AddContent("overnumType", GetLightsType(JLightsString_Tag[i].ToString(), "41"));
                ...
                if (checkednum_ssy)
                {
                    ...
                    // === 建立購買人資料 ===
                    ApplicantID = objLightDAC.Addapplicantinfo_lights_ssy(
                        Name: AppName,
                        Mobile: AppMobile,
                        Cost: "0",
                        County: Appcounty,
                    ...
                    if (ApplicantID > 0)
                    {
                        for (int i = 0; i < listcount; i++)
                        {
                            ...
                            string lightsType = GetLightsType(lightsString, "41");
                            ...
                            int cost = GetLightsCost(41, lightsType);

                            if (name != "")
                            {
                                lightsinfo = true;
                                LightsID = objLightDAC.AddLights_ssy(
