...
{
    /// <summary>
    /// 花蓮慈惠石壁部堂點燈服務頁面 (光明燈/太歲燈/求子燈/護子燈/文昌燈/特別貴人燈/特別健康燈/特別事業燈)
    /// </summary>
    /// <remarks>
    /// 此頁面繼承自 AjaxBasePage，負責：
    /// 1. 控制祈福燈顯示狀態（光明燈、太歲燈、求子燈、護子燈、文昌燈、特別貴人燈、特別健康燈、特別事業燈）
    /// 2. 初始化 Ajax 處理器（建立/修改報名資料）
    /// 3. 處理購買人與祈福人資料寫入流程
    /// </remarks>
    public partial class templeService_lights_sbbt : AjaxBasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                // 顯示求子燈
                this.light4.Visible = objLightDAC.CheckedLightsNum("44", adminID.ToString(), 1, Year, this);

                // 顯示護子燈
                this.light5.Visible = objLightDAC.CheckedLightsNum("45", adminID.ToString(), 1, Year, this);

                // 顯示特別貴人燈
                this.light6.Visible = objLightDAC.CheckedLightsNum("10", adminID.ToString(), 1, Year, this);

                // 顯示特別健康燈
                this.light7.Visible = objLightDAC.CheckedLightsNum("8", adminID.ToString(), 1, Year, this);

                // 顯示特別事業燈
                this.light8.Visible = objLightDAC.CheckedLightsNum("22", adminID.ToString(), 1, Year, this);
        ...
        public class AjaxPageHandler
        {
            ...
            public void gotochecked(BasePage basePage)
            {
                ...
                string postURL_Init = "Lights_sbbt_Index";
                string url = HttpContext.Current.Request.Url.AbsoluteUri;
                string postURL = GetRequestURL(url, postURL_Init);

                // === 確認各燈種數量 ===
                int[] count_sbbt_lights = new int[8];
                bool checkednum_sbbt = true;
                for (int i = 0; i < listcount; i++)
                {
                    switch (JLightsString_Tag[i].ToString())
                    {
                        case "光明燈": count_sbbt_lights[0]++; break;
                        case "太歲燈": count_sbbt_lights[1]++; break;
                        case "文昌燈": count_sbbt_lights[2]++; break;
                        case "求子燈": count_sbbt_lights[3]++; break;
                        case "護子燈": count_sbbt_lights[4]++; break;
                        case "特別貴人燈": count_sbbt_lights[5]++; break;
                        case "特別健康燈": count_sbbt_lights[6]++; break;
                        case "特別事業燈": count_sbbt_lights[7]++; break;
                    }
                }
                ...
                {
                    string lightsType = GetLightsType(JLightsString_Tag[i].ToString(), "39");
                    int c = 0;

                    c += lightsType == "3" ? count_sbbt_lights[0] : 0;
                    c += lightsType == "4" ? count_sbbt_lights[1] : 0;
                    c += lightsType == "5" ? count_sbbt_lights[2] : 0;
                    c += lightsType == "44" ? count_sbbt_lights[3] : 0;
                    c += lightsType == "45" ? count_sbbt_lights[4] : 0;
                    c += lightsType == "10" ? count_sbbt_lights[5] : 0;
                    c += lightsType == "8" ? count_sbbt_lights[6] : 0;
                    c += lightsType == "22" ? count_sbbt_lights[7] : 0;

                    if (objLightDAC.CheckedLightsNum(lightsType, AdminID.ToString(), c, Year, basePage))
                    {
                        checkednum_sbbt = false;
                        basePage.mJSonHelper.AddContent("overnumType", GetLightsType(JLightsString_Tag[i].ToString(), "39"));

                        // 若有 ad 參數允許強制通過
                        //if (basePage.Request["ad"] == "0610")
                        //{
                        //    checkednum_sbbt = true;
                        //}
                ...
                if (checkednum_sbbt)
                {
                    ...
                    // === 建立購買人資料 ===
                    ApplicantID = objLightDAC.Addapplicantinfo_lights_sbbt(
                        Name: AppName,
                        Mobile: AppMobile,
                        Cost: "0",
                        County: Appcounty,
                    ...
                    if (ApplicantID > 0)
                    {
                        for (int i = 0; i < listcount; i++)
                        {
                            ...
                            string lightsType = GetLightsType(lightsString, "39");
                            ...
                            {
                                lightsinfo = true;
                                LightsID = objLightDAC.AddLights_sbbt(
