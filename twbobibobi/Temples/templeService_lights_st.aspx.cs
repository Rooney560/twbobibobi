...
{
    /// <summary>
    /// 松柏嶺受天宮 點燈服務頁面 (光明燈/安奉太歲/文昌燈/財神燈)
    /// </summary>
    /// <remarks>
    /// 此頁面繼承自 AjaxBasePage，負責：
    /// 1. 控制祈福燈顯示狀態（光明燈、安奉太歲、文昌燈、財神燈）
    /// 2. 初始化 Ajax 處理器（建立/修改報名資料）
    /// 3. 處理購買人與祈福人資料寫入流程
    /// </remarks>
    public partial class templeService_lights_st : AjaxBasePage
    {
        ...
        public class AjaxPageHandler
        {
            ...
            public void gotochecked(BasePage basePage)
            {
                ...
                string postURL_Init = "Lights_st_Index";
                string url = HttpContext.Current.Request.Url.AbsoluteUri;
                string postURL = GetRequestURL(url, postURL_Init);

                // === 確認各燈種數量 ===
                int[] count_st_lights = new int[4];
                bool checkednum_st = true;
                for (int i = 0; i < listcount; i++)
                {
                    switch (JLightsString_Tag[i].ToString())
                    {
                        case "光明燈": count_st_lights[0]++; break;
                        case "安奉太歲": count_st_lights[1]++; break;
                        case "文昌燈": count_st_lights[2]++; break;
                        case "財神燈": count_st_lights[3]++; break;
                    }
                }
                ...
                {
                    string lightsType = GetLightsType(JLightsString_Tag[i].ToString(), "35");
                    int c = 0;

                    c += lightsType == "3" ? count_st_lights[0] : 0;
                    c += lightsType == "4" ? count_st_lights[1] : 0;
                    c += lightsType == "5" ? count_st_lights[2] : 0;
                    c += lightsType == "6" ? count_st_lights[3] : 0;

                    if (objLightDAC.CheckedLightsNum(lightsType, AdminID.ToString(), c, Year, basePage))
                    {
                        checkednum_st = false;
                        basePage.mJSonHelper.AddContent("overnumType", GetLightsType(JLightsString_Tag[i].ToString(), "35"));
                ...
                if (checkednum_st)
                {
                    ...
                    // === 建立購買人資料 ===
                    ApplicantID = objLightDAC.Addapplicantinfo_lights_st(
                        Name: AppName,
                        Mobile: AppMobile,
                        Cost: "0",
                        County: Appcounty,
                    ...
                    if (ApplicantID > 0)
                    {
                        for (int i = 0; i < listcount; i++)
                        {
                            ...
                            string lightsType = GetLightsType(lightsString, "35");
                            ...
                            {
                                lightsinfo = true;
                                LightsID = objLightDAC.AddLights_st(
