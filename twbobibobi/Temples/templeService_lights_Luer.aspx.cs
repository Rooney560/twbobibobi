...
{
    /// <summary>
    /// 台南正統鹿耳門聖母廟 點燈服務頁面 (光明燈/安太歲/文昌燈/姻緣燈/財利燈/福壽燈/寵物平安燈/月老姻緣燈)
    /// </summary>
    /// <remarks>
    /// 此頁面繼承自 AjaxBasePage，負責：
    /// 1. 控制祈福燈顯示狀態（光明燈、安太歲、文昌燈、姻緣燈、財利燈、福壽燈、寵物平安燈、月老姻緣燈）
    ...
    public partial class templeService_lights_Luer : AjaxBasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                // 顯示 財利燈
                this.light5.Visible = objLightDAC.CheckedLightsNum("9", adminID.ToString(), 1, Year, this);

                // 顯示 福壽燈
                this.light6.Visible = objLightDAC.CheckedLightsNum("11", adminID.ToString(), 1, Year, this);
        ...
        public class AjaxPageHandler
        {
            ...
            public void gotochecked(BasePage basePage)
            {
                ...
                string postURL_Init = "Lights_Luer_Index";
                string url = HttpContext.Current.Request.Url.AbsoluteUri;
                string postURL = GetRequestURL(url, postURL_Init);

                // === 確認各燈種數量 ===
                int[] count_Luer_lights = new int[6];
                bool checkednum_Luer = true;
                for (int i = 0; i < listcount; i++)
                {
                    switch (JLightsString_Tag[i].ToString())
                    {
                        case "光明燈": count_Luer_lights[0]++; break;
                        case "安太歲": count_Luer_lights[1]++; break;
                        case "文昌燈": count_Luer_lights[2]++; break;
                        case "姻緣燈": count_Luer_lights[3]++; break;
                        case "財利燈": count_Luer_lights[4]++; break;
                        case "福壽燈": count_Luer_lights[5]++; break;
                    }
                }
                ...
                {
                    string lightsType = GetLightsType(JLightsString_Tag[i].ToString(), "10");
                    int c = 0;

                    c += lightsType == "3" ? count_Luer_lights[0] : 0;
                    c += lightsType == "4" ? count_Luer_lights[1] : 0;
                    c += lightsType == "5" ? count_Luer_lights[2] : 0;
                    c += lightsType == "7" ? count_Luer_lights[0] : 0;
                    c += lightsType == "9" ? count_Luer_lights[1] : 0;
                    c += lightsType == "11" ? count_Luer_lights[2] : 0;

                    if (objLightDAC.CheckedLightsNum(lightsType, AdminID.ToString(), c, Year, basePage))
                    {
                        checkednum_Luer = false;
                        basePage.mJSonHelper.AddContent("overnumType", GetLightsType(JLightsString_Tag[i].ToString(), "10"));
                ...
                if (checkednum_Luer)
                {
                    ...
                    // === 建立購買人資料 ===
                    ApplicantID = objLightDAC.Addapplicantinfo_lights_Luer(
                        Name: AppName,
                        Mobile: AppMobile,
                        Cost: "0",
                        County: Appcounty,
                    ...
                    if (ApplicantID > 0)
                    {
                        for (int i = 0; i < listcount; i++)
                        {
                            ...
                            {
                                lightsinfo = true;
                                LightsID = objLightDAC.AddLights_Luer(
