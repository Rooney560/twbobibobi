...
{
    /// <summary>
    /// 北港武德宮 點燈服務頁面 (光明燈/安太歲/財神燈)
    /// </summary>
    /// <remarks>
    /// 此頁面繼承自 AjaxBasePage，負責：
    /// 1. 控制祈福燈顯示狀態（光明燈、安太歲、財神燈）
    /// 2. 初始化 Ajax 處理器（建立/修改報名資料）
    /// 3. 處理購買人與祈福人資料寫入流程
    /// </remarks>
    public partial class templeService_lights_wu : AjaxBasePage
    {
        ...
        public class AjaxPageHandler
        {
            ...
            public void gotochecked(BasePage basePage)
            {
                ...
                string postURL_Init = "Lights_wu_Index";
                ...
                for (int i = 0; i < listcount; i++)
                {
                    ...
                    {
                        case "光明燈": count_wu_lights[0]++; break;
                        case "太歲燈": count_wu_lights[1]++; break;
                        case "財神燈": count_wu_lights[2]++; break;
                    }
                }
                ...
                {
                    string lightsType = GetLightsType(JLightsString_Tag[i].ToString(), "6");
                    int c = 0;
                    ...
                    {
                        checkednum_wu = false;
                        basePage.mJSonHelper.AddContent("overnumType", GetLightsType(JLightsString_Tag[i].ToString(), "6"));
                ...
                if (checkednum_wu)
                {
                    ...
                    // === 建立購買人資料 ===
                    ApplicantID = objLightDAC.Addapplicantinfo_lights_wu(
                        Name: AppName,
                        Mobile: AppMobile,
                        Cost: "0",
                        County: Appcounty,
                    ...
                    if (ApplicantID > 0)
                    {
                        for (int i = 0; i < listcount; i++)
                        {
                            ...
                            string lightsType = GetLightsType(lightsString, "6");
                            ...
                            {
                                lightsinfo = true;
                                LightsID = objLightDAC.AddLights_wu(
