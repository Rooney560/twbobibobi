...
{
    public partial class templeService_purdue_wu : AjaxBasePage
    {
        public int aid = 0;
        public int a = 0;
        ...
        public class AjaxPageHandler
        {
            ...
            public void gotochecked(BasePage basePage)
            {
                ...
                string postURL = "Purdue_wu_Index";
                ...
                ApplicantID = objLightDAC.addapplicantinfo_purdue_wu(AppName, AppMobile, "0", "", "", "", "0", "N", "", "", 0, AdminID, postURL, Year);
                ...
                if (ApplicantID > 0)
                {
                    for (int i = 0; i < listcount; i++)
                    {
                        ...
                        {
                            purdueinfo = true;
                            //PurdueID = objLightDAC.addpurdue_wu(ApplicantID, name, Jmobile[i].ToString(), Jpurduetype[i].ToString(), purdueString, Jsex[i].ToString(), Jbirth[i].ToString(), homenum, Jemail[i].ToString(), Jcounty[i].ToString() + Jdist[i] + Jaddr[i].ToString(), Jaddr[i].ToString(), Jcounty[i].ToString(), Jdist[i].ToString(), JzipCode[i].ToString(), Jcount[i].ToString(), remark);

                            PurdueID = objLightDAC.addpurdue_wu(ApplicantID, name, Jmobile[i].ToString(), Jpurduetype[i].ToString(), purdueString, "1", birth, JleapMonth[i].ToString(), 
                                Jbirthtime[i].ToString(), birthMonth, age, Zodiac, Jsex[i].ToString(), Jhomenum[i].ToString(), Jemail[i].ToString(), Jcount[i].ToString(), 
                                Jaddr[i].ToString(), Jcounty[i].ToString(), Jdist[i].ToString(), JzipCode[i].ToString(), Jremark[i].ToString(), Year);
                        }
                    }
                }
            ...
            public void editinfo(BasePage basePage)
            {
                ...
                dtData = objLightDAC.Getpurdue_wu_info(applicantID, Year);
