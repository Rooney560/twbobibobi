...
{
    /// <summary>
    /// 桃園威天宮 點燈服務頁面 (光明燈/太歲燈/財神燈/藥師燈/貴人燈/福祿燈/智慧燈)
    /// </summary>
    /// <remarks>
    /// 此頁面繼承自 AjaxBasePage，負責：
    /// 1. 控制祈福燈顯示狀態（光明燈、太歲燈、財神燈、藥師燈、貴人燈、福祿燈、智慧燈）
    /// 2. 初始化 Ajax 處理器（建立/修改報名資料）
    /// 3. 處理購買人與祈福人資料寫入流程
    /// </remarks>
    public partial class templeService_lights_ty : AjaxBasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                // 顯示 藥師燈
                this.light4.Visible = objLightDAC.CheckedLightsNum("8", adminID.ToString(), 1, Year, this);

                // 顯示 貴人燈
                this.light5.Visible = objLightDAC.CheckedLightsNum("10", adminID.ToString(), 1, Year, this);
        ...
        public class AjaxPageHandler
        {
            ...
            public void gotochecked(BasePage basePage)
            {
                ...
                string postURL_Init = "Lights_ty_Index";
                ...
                for (int i = 0; i < listcount; i++)
                {
                    ...
                    {
                        case "光明燈": count_ty_lights[0]++; break;
                        case "太歲燈": count_ty_lights[1]++; break;
                        case "財神燈": count_ty_lights[2]++; break;
                        case "藥師燈": count_ty_lights[3]++; break;
                        case "貴人燈": count_ty_lights[4]++; break;
                        case "福祿燈": count_ty_lights[5]++; break;
                        case "智慧燈": count_ty_lights[6]++; break;
                    }
                }
                ...
                {
                    string lightsType = GetLightsType(JLightsString_Tag[i].ToString(), "14");
                    int c = 0;
                    ...
                    {
                        checkednum_ty = false;
                        basePage.mJSonHelper.AddContent("overnumType", GetLightsType(JLightsString_Tag[i].ToString(), "14"));
                ...
                if (checkednum_ty)
                {
                    ...
                    // === 建立購買人資料 ===
                    ApplicantID = objLightDAC.Addapplicantinfo_lights_ty(
                        Name: AppName,
                        Mobile: AppMobile,
                        Cost: "0",
                        County: Appcounty,
                    ...
                    if (ApplicantID > 0)
                    {
                        for (int i = 0; i < listcount; i++)
                        {
                            ...
                            {
                                lightsinfo = true;
                                LightsID = objLightDAC.AddLights_ty(
