    ...
    public partial class templeService_purdue_Lk : AjaxBasePage
    {
        ...
        public class AjaxPageHandler
        {
            ...
            public void gotochecked(BasePage basePage)
            {
                ...
                string postURL_Init = "Purdue_Lk_Index";

                string url = HttpContext.Current.Request.Url.AbsoluteUri;

                string postURL = GetRequestURL(url, postURL_Init);

                ApplicantID = objLightDAC.Addapplicantinfo_purdue_Lk(
                    Name: AppName,
                    Mobile: AppMobile,
                    Cost: "0",
                    County: Apprcounty,
                ...
                if (ApplicantID > 0)
                {
                    for (int i = 0; i < listcount; i++)
                    {
                        ...
                        int cost = GetPurdueCost(21, purdueType);

                        if (name != "")
                        {
                            purdueinfo = true;
                            PurdueID = objLightDAC.Addpurdue_Lk(
