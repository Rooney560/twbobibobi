...
{
    /// <summary>
    /// 五股賀聖宮點燈服務頁面 (光明燈/太歲燈/文昌燈/藥師燈/財利燈/玉皇燈)
    /// </summary>
    /// <remarks>
    /// 此頁面繼承自 AjaxBasePage，負責：
    /// 1. 控制祈福燈顯示狀態（光明燈、太歲燈、文昌燈、藥師燈、財利燈、玉皇燈）
    ...
    public partial class templeService_lights_Hs : AjaxBasePage
    {
        ...
        public class AjaxPageHandler
        {
            ...
            public void gotochecked(BasePage basePage)
            {
                ...
                string AppzipCode = basePage.Request["AppzipCode"];                                 //購買人郵政區號
                ...
                if (checkednum_Hs)
                {
                    ...
                    ApplicantID = objLightDAC.Addapplicantinfo_lights_Hs(
                        Name: AppName,
                        Mobile: AppMobile,
                        Cost: "0",
                        County: Appcounty,
                    ...
                    if (ApplicantID > 0)
                    {
                        for (int i = 0; i < listcount; i++)
                        {
                            ...
                            {
                                lightsinfo = true;
                                LightsID = objLightDAC.AddLights_Hs(
