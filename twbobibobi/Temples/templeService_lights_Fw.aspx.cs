...
{
    /// <summary>
    /// 斗六五路財神宮 點燈服務頁面 (光明燈(貴人燈)/安太歲/發財燈/月老桃花燈/消災延壽燈/寵物平安燈/財庫燈)
    /// </summary>
    /// <remarks>
    /// 此頁面繼承自 AjaxBasePage，負責：
    /// 1. 控制祈福燈顯示狀態（光明燈(貴人燈)、安太歲、發財燈、月老桃花燈、消災延壽燈、寵物平安燈、財庫燈）
    /// 2. 初始化 Ajax 處理器（建立/修改報名資料）
    /// 3. 處理購買人與祈福人資料寫入流程
    /// </remarks>
    public partial class templeService_lights_Fw : AjaxBasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                // 顯示 發財燈
                this.light3.Visible = objLightDAC.CheckedLightsNum("6", adminID.ToString(), 1, Year, this);

                // 顯示 月老桃花燈
                this.light4.Visible = objLightDAC.CheckedLightsNum("7", adminID.ToString(), 1, Year, this);

                // 顯示 消災延壽燈
                this.light5.Visible = objLightDAC.CheckedLightsNum("8", adminID.ToString(), 1, Year, this);

                // 顯示 寵物平安燈
                this.light6.Visible = objLightDAC.CheckedLightsNum("12", adminID.ToString(), 1, Year, this);

                // 顯示 財庫燈
                this.light7.Visible = objLightDAC.CheckedLightsNum("19", adminID.ToString(), 1, Year, this);
        ...
        public class AjaxPageHandler
        {
            ...
            public void gotochecked(BasePage basePage)
            {
                ...
                string postURL_Init = "Lights_Fw_Index";
                string url = HttpContext.Current.Request.Url.AbsoluteUri;
                string postURL = GetRequestURL(url, postURL_Init);

                // === 確認各燈種數量 ===
                int[] count_Fw_lights = new int[7];
                bool checkednum_da = true;
                for (int i = 0; i < listcount; i++)
                {
                    switch (JLightsString_Tag[i].ToString())
                    {
                        case "貴人燈": count_Fw_lights[0]++; break;
                        case "安太歲": count_Fw_lights[1]++; break;
                        case "發財燈": count_Fw_lights[2]++; break;
                        case "月老桃花燈": count_Fw_lights[3]++; break;
                        case "消災延壽燈": count_Fw_lights[4]++; break;
                        case "寵物平安燈": count_Fw_lights[5]++; break;
                        case "財庫燈": count_Fw_lights[6]++; break;
                    }
                }
                ...
                {
                    string lightsType = GetLightsType(JLightsString_Tag[i].ToString(), "15");
                    int c = 0;

                    c += lightsType == "3" ? count_Fw_lights[0] : 0;
                    c += lightsType == "4" ? count_Fw_lights[1] : 0;
                    c += lightsType == "6" ? count_Fw_lights[2] : 0;
                    c += lightsType == "7" ? count_Fw_lights[3] : 0;
                    c += lightsType == "8" ? count_Fw_lights[4] : 0;
                    c += lightsType == "12" ? count_Fw_lights[5] : 0;
                    c += lightsType == "19" ? count_Fw_lights[6] : 0;

                    if (objLightDAC.CheckedLightsNum(lightsType, AdminID.ToString(), c, Year, basePage))
                    {
                        checkednum_da = false;
                        basePage.mJSonHelper.AddContent("overnumType", GetLightsType(JLightsString_Tag[i].ToString(), "15"));
                ...
                if (checkednum_da)
                {
                    ...
                    // === 建立購買人資料 ===
                    ApplicantID = objLightDAC.Addapplicantinfo_lights_Fw(
                        Name: AppName,
                        Mobile: AppMobile,
                        Cost: "0",
                        County: Appcounty,
                    ...
                    if (ApplicantID > 0)
                    {
                        for (int i = 0; i < listcount; i++)
                        {
                            ...
                            string lightsType = GetLightsType(lightsString, "15");
                            ...
                            {
                                lightsinfo = true;
                                LightsID = objLightDAC.AddLights_Fw(
            ...
            public void editinfo(BasePage basePage)
            {
                ...
                dtData = objLightDAC.Getlights_Fw_Info(applicantID, Year);
