...
{
    /// <summary>
    /// 西螺福興宮 點燈服務頁面 (光明燈/安太歲/文昌燈/財神燈/藥師佛燈/觀音佛祖燈)
    /// </summary>
    /// <remarks>
    /// 此頁面繼承自 AjaxBasePage，負責：
    /// 1. 控制祈福燈顯示狀態（光明燈、安太歲、文昌燈、財神燈、藥師佛燈、觀音佛祖燈）
    /// 2. 初始化 Ajax 處理器（建立/修改報名資料）
    /// 3. 處理購買人與祈福人資料寫入流程
    /// </remarks>
    public partial class templeService_lights_Fu : AjaxBasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                // 顯示 觀音佛祖燈
                this.light6.Visible = objLightDAC.CheckedLightsNum("24", adminID.ToString(), 1, Year, this);
        ...
        public class AjaxPageHandler
        {
            ...
            public void gotochecked(BasePage basePage)
            {
                ...
                string postURL_Init = "Lights_Fu_Index";
                string url = HttpContext.Current.Request.Url.AbsoluteUri;
                string postURL = GetRequestURL(url, postURL_Init);

                // === 確認各燈種數量 ===
                int[] count_Fu_lights = new int[6];
                bool checkednum_da = true;
                for (int i = 0; i < listcount; i++)
                {
                    switch (JLightsString_Tag[i].ToString())
                    {
                        case "光明燈": count_Fu_lights[0]++; break;
                        case "安太歲": count_Fu_lights[1]++; break;
                        case "文昌燈": count_Fu_lights[2]++; break;
                        case "財神燈": count_Fu_lights[3]++; break;
                        case "藥師佛燈": count_Fu_lights[4]++; break;
                        case "觀音佛祖燈": count_Fu_lights[5]++; break;
                    }
                }
                ...
                {
                    string lightsType = GetLightsType(JLightsString_Tag[i].ToString(), "8");
                    int c = 0;

                    c += lightsType == "3" ? count_Fu_lights[0] : 0;
                    c += lightsType == "4" ? count_Fu_lights[1] : 0;
                    c += lightsType == "5" ? count_Fu_lights[2] : 0;
                    c += lightsType == "6" ? count_Fu_lights[3] : 0;
                    c += lightsType == "8" ? count_Fu_lights[4] : 0;
                    c += lightsType == "24" ? count_Fu_lights[5] : 0;

                    if (objLightDAC.CheckedLightsNum(lightsType, AdminID.ToString(), c, Year, basePage))
                    {
                        checkednum_da = false;
                        basePage.mJSonHelper.AddContent("overnumType", GetLightsType(JLightsString_Tag[i].ToString(), "8"));
                ...
                if (checkednum_da)
                {
                    ...
                    // === 建立購買人資料 ===
                    ApplicantID = objLightDAC.Addapplicantinfo_lights_Fu(
                        Name: AppName,
                        Mobile: AppMobile,
                        Cost: "0",
                        County: Appcounty,
                    ...
                    if (ApplicantID > 0)
                    {
                        for (int i = 0; i < listcount; i++)
                        {
                            ...
                            string lightsType = GetLightsType(lightsString, "8");
                            ...
                            {
                                lightsinfo = true;
                                LightsID = objLightDAC.AddLights_Fu(
            ...
            public void editinfo(BasePage basePage)
            {
                ...
                dtData = objLightDAC.Getlights_Fu_Info(applicantID, Year);
