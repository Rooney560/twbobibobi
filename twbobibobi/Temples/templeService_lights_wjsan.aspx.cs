...
{
    /// <summary>
    /// 台灣道教總廟無極三清總道院 點燈服務頁面 (光明燈/太歲燈/文昌燈/財神燈/藥師燈/事業燈)
    /// </summary>
    /// <remarks>
    /// 此頁面繼承自 AjaxBasePage，負責：
    /// 1. 控制祈福燈顯示狀態（光明燈、太歲燈、文昌燈、財神燈、藥師燈、事業燈）
    ...
    public partial class templeService_lights_wjsan : AjaxBasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                // 顯示 事業燈
                this.light6.Visible = objLightDAC.CheckedLightsNum("22", adminID.ToString(), 1, Year, this);
        ...
        public class AjaxPageHandler
        {
            ...
            public void gotochecked(BasePage basePage)
            {
                ...
                string postURL_Init = "Lights_wjsan_Index";
                string url = HttpContext.Current.Request.Url.AbsoluteUri;
                string postURL = GetRequestURL(url, postURL_Init);

                // === 確認各燈種數量 ===
                int[] count_wjsan_lights = new int[6];
                bool checkednum_wjsan = true;
                for (int i = 0; i < listcount; i++)
                {
                    switch (JLightsString_Tag[i].ToString())
                    {
                        case "光明燈": count_wjsan_lights[0]++; break;
                        case "太歲燈": count_wjsan_lights[1]++; break;
                        case "文昌燈": count_wjsan_lights[2]++; break;
                        case "財神燈": count_wjsan_lights[3]++; break;
                        case "藥師燈": count_wjsan_lights[4]++; break;
                        case "事業燈": count_wjsan_lights[5]++; break;
                    }
                }
                ...
                {
                    string lightsType = GetLightsType(JLightsString_Tag[i].ToString(), "31");
                    int c = 0;

                    c += lightsType == "3" ? count_wjsan_lights[0] : 0;
                    c += lightsType == "4" ? count_wjsan_lights[1] : 0;
                    c += lightsType == "5" ? count_wjsan_lights[2] : 0;
                    c += lightsType == "6" ? count_wjsan_lights[0] : 0;
                    c += lightsType == "8" ? count_wjsan_lights[1] : 0;
                    c += lightsType == "22" ? count_wjsan_lights[2] : 0;

                    if (objLightDAC.CheckedLightsNum(lightsType, AdminID.ToString(), c, Year, basePage))
                    {
                        checkednum_wjsan = false;
                        basePage.mJSonHelper.AddContent("overnumType", GetLightsType(JLightsString_Tag[i].ToString(), "31"));
                ...
                if (checkednum_wjsan)
                {
                    ...
                    // === 建立購買人資料 ===
                    ApplicantID = objLightDAC.Addapplicantinfo_lights_wjsan(
                        Name: AppName,
                        Mobile: AppMobile,
                        Cost: "0",
                        County: Appcounty,
                    ...
                    if (ApplicantID > 0)
                    {
                        for (int i = 0; i < listcount; i++)
                        {
                            ...
                            {
                                lightsinfo = true;
                                LightsID = objLightDAC.AddLights_wjsan(
