        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                ...
                if (dtCharge.Rows.Count > 0)
                {
                    ...
                    if (status == 0)
                    {
                        ...
                        if (result[0] == "1" || result[0] == "2")
                        {
                            if (result.Length > 1)
                            {
                                ...
                                objDatabaseHelper.Updateapplicantinfo_Lights_Luer(aid, cost, 2, Year);
                                ...
                                if (objDatabaseHelper.UpdateChargeLog_Lights_Luer(orderId, tid, msg, Request.UserHostAddress, CallbackLog, Year, ref ChargeType))
                                {
                                    if (objSMSHepler.SendMsg_SL(mobile, msg))
                                    {
                                        ...
                                        m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=1&a=" + adminID + "&aid=" + aid + (ChargeType == "Twm" && type == 20 ? "&type=3" : "") + (ChargeType == "Twm" ? "&twm=1" : "");
                    ...
                    {
                        //已經付費成功。
                        DataTable dtapplicantinfo = objDatabaseHelper.Getapplicantinfo_Lights_Luer(aid, 10, Year);
                        m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=1&a=10&aid=" + aid + (type == 20 ? "&type=3" : "") + (dtCharge.Rows[0]["ChargeType"].ToString() == "Twm" ? "&twm=1" : "");
                        Response.Redirect(m2, true);
                    }
                    else
                    {
