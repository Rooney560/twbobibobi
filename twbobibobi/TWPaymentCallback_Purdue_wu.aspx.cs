...
namespace Temple
{
    public partial class TWPaymentCallback_Purdue_wu : BasePage
    {
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                ...
                if (dtCharge.Rows.Count > 0)
                {
                    ...
                    if (status == 0)
                    {
                        ...
                        string rebackURL = "https://bobibobi.tw/Temples/templeService_purdue_wu.aspx";
                        ...
                        if (result[0] == "1" || result[0] == "2")
                        {
                            if (result.Length > 1)
                            {
                                ...
                                //取得申請人資料表
                                //DataTable dtapplicantinfo = objDatabaseHelper.Getapplicantinfo_Purdue_wu(aid, adminID, Year);
                                ////更新購買表內購買人狀態為已付款(Status=2)
                                //int cost = dtapplicantinfo.Rows.Count > 0 ? int.Parse(dtapplicantinfo.Rows[0]["Cost"].ToString()) : 0;
                                objDatabaseHelper.Updateapplicantinfo_Purdue_wu(aid, cost, 2, Year);
                                ...
                                ////更新普渡資料表並取得訂單編號
                                //objDatabaseHelper.UpdatePurdue_wu_Info(aid, ref purduelist);
                                ////取得申請人資料表
                                //DataTable dtapplicantinfo = objDatabaseHelper.Getapplicantinfo_Purdue_wu(aid, adminID);
                                ////更新購買表內購買人狀態為已付款(Status=2)
                                //objDatabaseHelper.Updateapplicantinfo_Purdue_wu(aid);
                                ...
                                ////更新流水付費表資訊(付費成功)
                                //if (objDatabaseHelper.UpdateChargeLog_Purdue_wu(orderId, tid, msg, Request.UserHostAddress, CallbackLog))
                                //{
                                //    if (objSMSHepler.SendMsg_SL(mobile, msg))
                                //    {
                                //        //m2 = m2.IndexOf("aid=") > 0 ? m2 : (m2.IndexOf("?") > 0 ? m2 + "&aid=" + m1 : m2 + "?aid=" + m1 + "&a=" + adminID);
                                ...
                                //    else
                                //    {
                                //        Response.Write("<script>alert('傳送簡訊失敗。請聯繫管理員。客服電話：04-23582760。');window.location.href='https://bobibobi.tw/Temples/templeService_purdue.aspx?a=6'</script>");
                                //    }
                                //}
                                //else
                                //{
                                //    Response.Write("<script>alert('付款過程失敗。請聯繫管理員。客服電話：04-23582760。');window.location.href='https://bobibobi.tw/Temples/templeService_purdue.aspx?a=6'</script>");
                                //}
                            }
                            else
                            {
                        ...
                        {
                            if (objDatabaseHelper.UpdateChargeStatus_Purdue_wu(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                            {
                        ...
                        {
                            objDatabaseHelper.UpdateChargeStatus_Purdue_wu(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
                    ...
                    {
                        //已經付費成功。
                        m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=2&a=6&aid=" + aid + (dtCharge.Rows[0]["ChargeType"].ToString() == "Twm" ? "&twm=1" : "");
                        Response.Redirect(m2, true);
                    }
                    else
                    {
                        SaveErrorLog(resp + ", 此訂單已交易失敗!");
                        Response.Write("<script>alert('此訂單已交易失敗，交易代碼：" + resp + "如有疑問。請洽客服電話：04-23582760。');window.location.href='https://bobibobi.tw/Temples/templeInfo.aspx?a=6'</script>");
                    }
                }
                else
                {
                    ...
                    Response.Write("<script>alert('取得付款資料失敗，錯誤代碼：" + resp + "。客服電話：04-23582760。');window.location.href='https://bobibobi.tw/Temples/templeInfo.aspx?a=6'</script>");
                }

                //Response.Write("<script>alert('取得付款資料失敗，錯誤代碼：" + resp + "。客服電話：04-23582760。');window.location.href='https://bobibobi.tw/Temples/templeService_purdue.aspx?a=6'</script>");
            }
            else
            {
