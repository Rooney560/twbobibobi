    ...
    public partial class TWPaymentCallback_Supplies_dh : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                ...
                int status = 999;
                int adminID = objDatabaseHelper.GetAdminID_Supplies_dh(aid, Year);

                DataTable dtCharge = objDatabaseHelper.GetChargeLog_Supplies_dh(orderId, Year);
                ...
                if (dtCharge.Rows.Count > 0)
                {
                    ...
                    if (status == 0)
                    {
                        ...
                        if (result[0] == "1" || result[0] == "2")
                        {
                            ...
                            objDatabaseHelper.UpdateSupplies_dh_Info(aid, Year, ref Supplieslist);
                            //DataTable dtapplicantinfo = objDatabaseHelper.Getapplicantinfo_Supplies_dh(aid, adminID, Year);
                            //int cost = dtapplicantinfo.Rows.Count > 0 ? int.Parse(dtapplicantinfo.Rows[0]["Cost"].ToString()) : 0;
                            objDatabaseHelper.Updateapplicantinfo_Supplies_dh(aid, cost, 2, Year); //更新購買表內購買人狀態為已付款(Status=2)
                            ...
                            //更新流水付費表資訊(付費成功)
                            if (objDatabaseHelper.UpdateChargeLog_Supplies_dh(orderId, tid, msg, Request.UserHostAddress, CallbackLog, Year, ref ChargeType))
                            {
                                if (objSMSHepler.SendMsg_SL(mobile, msg))
                                {
                                    //m2 = m2.IndexOf("aid=") > 0 ? m2 : (m2.IndexOf("?") > 0 ? m2 + "&aid=" + m1 : m2 + "?aid=" + m1 + "&a=" + adminID);
                                    m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?a=" + adminID + "&aid=" + aid + "&kind=11" + (ChargeType == "Twm" ? "&twm=1" : "");
                        ...
                        {
                            if (objDatabaseHelper.UpdateChargeStatus_Supplies_dh(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                            {
                        ...
                        {
                            objDatabaseHelper.UpdateChargeStatus_Supplies_dh(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
                            //objDatabaseHelper.UpdateChargeErrLog_Supplies_dh(orderId, "", Request.UserHostAddress, CallbackLog, Year);
                    ...
                    {
                        //已經付費成功。
                        m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=11&a=" + adminID + "&aid=" + aid + (dtCharge.Rows[0]["ChargeType"].ToString() == "Twm" ? "&twm=1" : "");
                        Response.Redirect(m2, true);
                    }
                    else
                    {
