...
namespace Temple
{
    public partial class TWPaymentCallback_Supplies_wu3 : BasePage
    {
        /// <summary>
        /// 
        /// </summary>
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                try
                {
                    ...
                    if (dtCharge.Rows.Count > 0)
                    {
                        ...
                        string rebackURL = "https://bobibobi.tw/Temples/templeService_supplies3.aspx";
                        ...
                        if (status == 0)
                        {
                            ...
                            if (result[0] == "1" || result[0] == "2")
                            {
                                ...
                                string msg = "【遠傳交易成功通知】您購買遠傳祈福法會企業補財庫服務，每月交易金額1300元，購買後可至【訂單查詢】查詢交易紀錄,遠傳祝您順心平安!";

                                objLightDAC.UpdateSupplies_wu_Info3(aid, Year, ref msg, ref supplieslist, ref Supplieslist);
                                ...
                                if (objLightDAC.UpdateChargeLog_Supplies_wu3(orderId, tid, msg, Request.UserHostAddress, CallbackLog, Year, ref ChargeType, ref uStatus))
                                {
                                    if (invStatus)
                                    {
                                        try
                                        {
                                            ...
                                            DataTable dtApplicantInfo = objALightDAC.GetAPPCharge_wu_Supplies3(aid, Year);
                                            ...
                                            if (dtApplicantInfo.Rows.Count > 0)
                                            {
                                                ...
                                                if (rs.Success)
                                                {
                                                    ...
                                                    if (objLightDAC.UpdateInvoiceDetail(aid, adminId, 6, invoiceNo, rawJson, "1", Year))
                                                    {
                                                        if (InvoiceEmailSender.Send(rs, Emailitems, buyerEmail, buyerName, invoiceRow["BuyerIdentifier"]?.ToString() ?? "0000000000", NumString, cost, dtNow, YearROC, Month, Date, Time))
                                                        {
                                                            ...
                                                            {
                                                                // ❌ 傳送簡訊失敗，記錄錯誤
                                                                SaveErrorLog("TWPaymentCallback_Supplies_wu3" + $"傳送簡訊失敗");
                                                        ...
                                                        {
                                                            // ❌ 寄送EMAIL失敗，記錄錯誤
                                                            SaveErrorLog("TWPaymentCallback_Supplies_wu3" + $"寄送EMAIL失敗");
                                                        }
                                                    }
                                                    else
                                                    {
                                                        // ❌ 發票失敗，記錄錯誤
                                                        SaveErrorLog("TWPaymentCallback_Supplies_wu3" + $"更新發票失敗 AdminID: {adminId}" + new Exception(rs.ErrorMessage));
                                                    }
                                                }
                                                else
                                                {
                                                    // ❌ 發票開立失敗 → 只寫 Log，不影響訂單流程
                                                    SaveErrorLog("TWPaymentCallback_Supplies_wu3 開立發票失敗 AdminID="
                                            ...
                                            {
                                                // ❌ 個別筆數處理例外錯誤
                                                SaveErrorLog("TWPaymentCallback_Supplies_wu3" + $"開立發票處理例外, 取得訂單錯誤。");
                                            }
                                        ...
                                        {
                                            // ❌ 個別筆數處理例外錯誤
                                            SaveErrorLog("TWPaymentCallback_Supplies_wu3" + $"開立發票處理例外 ex: " + ex.InnerException.Message);
                                        }
                                ...
                                {
                                    // ❌ 更新流水付費表資訊失敗，記錄錯誤
                                    SaveErrorLog("TWPaymentCallback_Supplies_wu3" + $"更新流水付費表資訊失敗");
                            ...
                            else if (result[0] == "4")
                            {
                                ...
                                {
                                    // ❌ 此用戶已退款，記錄錯誤
                                    SaveErrorLog("TWPaymentCallback_Supplies_wu3" + $"此用戶已退款");
                            ...
                            else
                            {
                                ...
                                {
                                    // ❌ 付款失敗，記錄錯誤
                                    SaveErrorLog("TWPaymentCallback_Supplies_wu3" + $"付款失敗，錯誤代碼：" + result[0]);
