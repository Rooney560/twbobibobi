        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                ...
                if (dtCharge.Rows.Count > 0)
                {
                    ...
                    if (status == 0)
                    {
                        ...
                        if (result[0] == "1" || result[0] == "2")
                        {
                            if (result.Length > 1)
                            {
                                ...
                                objDatabaseHelper.UpdateSupplies_ma_Info(aid, Year, ref msg, ref supplieslist);
                                //objDatabaseHelper.UpdateSupplies_ma_Info(aid, Year, ref Supplieslist);
                                //DataTable dtapplicantinfo = objDatabaseHelper.Getapplicantinfo_Supplies_ma(aid, adminID, Year);
                                //int cost = dtapplicantinfo.Rows.Count > 0 ? int.Parse(dtapplicantinfo.Rows[0]["Cost"].ToString()) : 0;
                                objDatabaseHelper.Updateapplicantinfo_Supplies_ma(aid, cost, 2, Year); //更新購買表內購買人狀態為已付款(Status=2)
                        ...
                        {
                            if (objDatabaseHelper.UpdateChargeStatus_Supplies_ma(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                            {
                        ...
                        {
                            objDatabaseHelper.UpdateChargeStatus_Supplies_ma(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
                            //objDatabaseHelper.UpdateChargeErrLog_Supplies_ma(orderId, "", Request.UserHostAddress, CallbackLog, Year);
                    ...
                    {
                        //已經付費成功。
                        m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=7&a=" + adminID + "&aid=" + aid + (dtCharge.Rows[0]["ChargeType"].ToString() == "Twm" ? "&twm=1" : "");
                        Response.Redirect(m2, true);
                    }
                    else
                    {
