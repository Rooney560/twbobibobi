                ...
                if (dtCharge.Rows.Count > 0)
                {
                    ...
                    if (status == 0)
                    {
                        ...
                        if (result[0] == "1" || result[0] == "2")
                        {
                            ...
                            //if (objDatabaseHelper.UpdateChargeLog_Supplies_wu2(orderId, tid, msg, Request.UserHostAddress, CallbackLog))
                            //{
                            ...
                            //            data = myArrayList[i].ToString();

                            //            objSMSHepler.SendMsg_SL(mobile, data);
                            ...
                            //    m2 = "https://bobibobi.tw/SearchLog.aspx" + "?aid=" + m1 + "&a=" + adminID + "&type=2&kind=4";
                            //    //m2 = m2.IndexOf("aid=") > 0 ? m2 : (m2.IndexOf("?") > 0 ? m2 + "&aid=" + m1 : m2 + "?aid=" + m1 + "&a=" + adminID);
                            ...
                            //else
                            //{
                            //    Response.Write("<script>alert('付款過程失敗。請聯繫管理員。錯誤代碼：109');window.location.href='https://bobibobi.tw/Supplies_wude2.aspx'</script>");
                            //}
                        }
                        else if (result[0] == "4")
                        {
                    ...
                    //else if ((int)dtCharge.Rows[0]["Status"] == 1)
                    //{
                    //    if(resp.IndexOf("twm") >= 0)
                    //    {
                    //        rebackURL += "?twm=1";
                    //    }
                    //    Response.Write("<script>alert('此訂單已付款完成！');window.location.href='"+ rebackURL + "'</script>");
                    //}
                    else if (status == 1)
                    {
                        //已經付費成功。
