            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                ...
                if (dtCharge.Rows.Count > 0)
                {
                    ...
                    if (status == 0)
                    {
                            ...
                            if (result.Length > 1)
                            {
                                ...
                                if (objDatabaseHelper.UpdateChargeLog_Lights_da(orderId, tid, msg, Request.UserHostAddress, CallbackLog, Year, ref ChargeType))
                                {
                                    if (objSMSHepler.SendMsg_SL(mobile, msg))
                                    {
                                        ...
                                        //{
                                        //    m2 = "https://bobibobi.tw/Temples/templeComplete_in.aspx?kind=1&a=" + adminID + "&aid=" + aid + (ChargeType == "Twm" ? "&twm=1" : "");
                                        //}
                        ...
                        {
                            if (objDatabaseHelper.UpdateChargeStatus_Lights_da(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                            {
                        ...
                        {
                            objDatabaseHelper.UpdateChargeStatus_Lights_da(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
