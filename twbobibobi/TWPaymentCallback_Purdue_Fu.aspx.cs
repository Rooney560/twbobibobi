...
namespace Temple
{
    public partial class TWPaymentCallback_Purdue_Fu : BasePage
    {
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                ...
                if (dtCharge.Rows.Count > 0)
                {
                    ...
                    if (status == 0)
                    {
                        ...
                        string rebackURL = "https://bobibobi.tw/Temples/templeService_purdue_Fu.aspx";
                        ...
                        if (result[0] == "1" || result[0] == "2")
                        {
                            if (result.Length > 1)
                            {
                                ...
                                //取得申請人資料表
                                //DataTable dtapplicantinfo = objDatabaseHelper.Getapplicantinfo_Purdue_Fu(aid, adminID, Year);
                                ////更新購買表內購買人狀態為已付款(Status=2)
                                //int cost = dtapplicantinfo.Rows.Count > 0 ? int.Parse(dtapplicantinfo.Rows[0]["Cost"].ToString()) : 0;
                                objDatabaseHelper.Updateapplicantinfo_Purdue_Fu(aid, cost, 2, Year);
                                ...
                                ////更新普渡資料表並取得訂單編號
                                //objDatabaseHelper.UpdatePurdue_Fu_Info(aid, ref purduelist);
                                ////取得申請人資料表
                                //DataTable dtapplicantinfo = objDatabaseHelper.Getapplicantinfo_Purdue_Fu(aid, adminID);
                                ////更新購買表內購買人狀態為已付款(Status=2)
                                //objDatabaseHelper.Updateapplicantinfo_Purdue_Fu(aid);
                                ...
                                ////更新流水付費表資訊(付費成功)
                                //if (objDatabaseHelper.UpdateChargeLog_Purdue_Fu(orderId, tid, msg, Request.UserHostAddress, CallbackLog))
                                //{
                                //    if (objSMSHepler.SendMsg_SL(mobile, msg))
                                //    {
                                ...
                                //    else
                                //    {
                                //        Response.Write("<script>alert('傳送簡訊失敗。請聯繫管理員。客服電話：04-36092299。');window.location.href='https://bobibobi.tw/Temples/templeService_purdue.aspx?a=8'</script>");
                                //    }
                                //}
                                //else
                                //{
                                //    Response.Write("<script>alert('付款過程失敗。請聯繫管理員。客服電話：04-36092299。');window.location.href='https://bobibobi.tw/Temples/templeService_purdue.aspx?a=8'</script>");
                                //}
                            }
                            else
                            {
                        ...
                        {
                            if (objDatabaseHelper.UpdateChargeStatus_Purdue_Fu(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                            {
                                Response.Write("<script>alert('此用戶已退款。');window.location.href='" + rebackURL + "'</script>");
                            }
                            //if (objDatabaseHelper.UpdateChargeStatus_Purdue_Fu(orderId, -2))
                            //{
                            //    Response.Write("<script>alert('此用戶已退款。');window.location.href='https://bobibobi.tw/Temples/templeService_purdue.aspx?a=8'</script>");
                            //}
                        }
                        else
                        {
                            objDatabaseHelper.UpdateChargeStatus_Purdue_Fu(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
                            ...
                            //objDatabaseHelper.UpdateChargeErrLog_Purdue_Fu(orderId, "", Request.UserHostAddress, CallbackLog);
                            ...
                            //else
                            //{
                            //    Response.Write("<script>alert('付款失敗，錯誤代碼：" + result[0] + "。客服電話：04-36092299。');window.location.href='https://bobibobi.tw/Temples/templeService_purdue.aspx?a=8'</script>");
                            //}
                        }
                    }
                    else if (status == 1)
                    {
                        //已經付費成功。
                        m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=2&a=8&aid=" + aid + (dtCharge.Rows[0]["ChargeType"].ToString() == "Twm" ? "&twm=1" : "");
                        Response.Redirect(m2, true);
                    }
                    else
                    {
                        SaveErrorLog(resp + ", 此訂單已交易失敗!");
                        Response.Write("<script>alert('此訂單已交易失敗，交易代碼：" + resp + "如有疑問。請洽客服電話：04-36092299。');window.location.href='https://bobibobi.tw/Temples/templeInfo.aspx?a=8'</script>");
                    }
                ...
                else
                {
                    ...
                    Response.Write("<script>alert('取得付款資料失敗，錯誤代碼：" + resp + "。客服電話：04-36092299。');window.location.href='https://bobibobi.tw/Temples/templeInfo.aspx?a=8'</script>");
                }
            }
            else
            {
