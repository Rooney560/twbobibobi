                        ...
                        if (status == 0)
                        {
                            ...
                            if (result[0] == "1" || result[0] == "2")
                            {
                                if (result.Length > 1)
                                {
                                    ...
                                    //取得申請人資料表
                                    //DataTable dtapplicantinfo = objDatabaseHelper.Getapplicantinfo_Purdue_ty(aid, adminID, Year);
                                    ////更新購買表內購買人狀態為已付款(Status=2)
                                    //int cost = dtapplicantinfo.Rows.Count > 0 ? int.Parse(dtapplicantinfo.Rows[0]["Cost"].ToString()) : 0;
                                    objDatabaseHelper.Updateapplicantinfo_Purdue_ty(aid, cost, 2, Year);
                            ...
                            {
                                if (objDatabaseHelper.UpdateChargeStatus_Purdue_ty(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                                {
                            ...
                            {
                                objDatabaseHelper.UpdateChargeStatus_Purdue_ty(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
