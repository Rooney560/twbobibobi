...
{
    public partial class TWPaymentCallback_Blessing_st : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                try
                {
                    ...
                    Session["PaymentAuthKey"] = "?kind=" + kind + "&a=35&aid=" + aid;

                    DataTable dtCharge = objLightDAC.GetChargeLog_Blessing_st(orderId, Year);
                    ...
                    if (dtCharge.Rows.Count > 0)
                    {
                        ...
                        if (status == 0)
                        {
                            int type = objLightDAC.GetBlessingType_st(aid, Year);

                            string rebackURL = "https://bobibobi.tw/Temples/templeService_blessing_st.aspx";
                            ...
                            if (result[0] == "1" || result[0] == "2")
                            {
                                if (result.Length > 1)
                                {
                                    ...
                                    //更新普渡資料表並取得訂單編號
                                    objLightDAC.UpdateBlessing_st_Info(aid, Year, ref msg, ref blessinglist, ref Blessinglist);
                                    ////更新購買表內購買人狀態為已付款(Status=2)
                                    objLightDAC.Updateapplicantinfo_Blessing_st(aid, cost, 2, Year);

                                    SMSHepler objSMSHepler = new SMSHepler();
                                    string ChargeType = string.Empty;
                                    int uStatus = 0;
                                    //更新流水付費表資訊(付費成功)
                                    if (objLightDAC.UpdateChargeLog_Blessing_st(orderId, tid, msg, Request.UserHostAddress, CallbackLog, Year, ref ChargeType, ref uStatus))
                                    {
                                        if (invStatus)
                                        {
                                            try
                                            {
                                                ...
                                                DataTable dtApplicantInfo = objALightDAC.GetAPPCharge_st_Blessing(aid, Year);
                                                ...
                                                if (dtApplicantInfo.Rows.Count > 0)
                                                {
                                                    ...
                                                    NumString += string.Join(",", Blessinglist);

                                                    foreach (DataRow row in dtApplicantInfo.Rows)
                                                    {
                                                        string BlessingType = row["BlessingType"].ToString();
                                                        string BlessingString = row["BlessingString"].ToString();
                                                        string description = $"線上服務費({templeName}-{BlessingString})";
                                                        ...
                                                        // 取單價
                                                        int price = InvoiceHelper.GetUnitPrice(adminId, int.Parse(kind), BlessingType);
                                                        ...
                                                        {
                                                            string errormsg = $"❌ 金額不符，OrderID={row["OrderID"]}, Kind={kind}, Service={BlessingString}, " +
                                                        ...
                                                        {
                                                            Description = "線上服務費",
                                                            ProductName = $"{templeName}-{BlessingString}",
                                                            Quantity = quantity,
                                                    ...
                                                    if (rs.Success)
                                                    {
                                                        ...
                                                        if (objLightDAC.UpdateInvoiceDetail(aid, adminId, 23, invoiceNo, rawJson, "1", Year))
                                                        {
                                                            if (InvoiceEmailSender.Send(rs, Emailitems, buyerEmail, buyerName, invoiceRow["BuyerIdentifier"]?.ToString() ?? "0000000000", NumString, cost, dtNow, YearROC, Month, Date, Time))
                                                            {
                                                                ...
                                                                {
                                                                    // ❌ 傳送簡訊失敗，記錄錯誤
                                                                    SaveErrorLog("TWPaymentCallback_Blessing_st" + $"傳送簡訊失敗");
                                                            ...
                                                            {
                                                                // ❌ 寄送EMAIL失敗，記錄錯誤
                                                                SaveErrorLog("TWPaymentCallback_Blessing_st" + $"寄送EMAIL失敗");
                                                            }
                                                        }
                                                        else
                                                        {
                                                            // ❌ 發票失敗，記錄錯誤
                                                            SaveErrorLog("TWPaymentCallback_Blessing_st" + $"更新發票失敗 AdminID: {adminId}" + new Exception(rs.ErrorMessage));
                                                        }
                                                    }
                                                    else
                                                    {
                                                        // ❌ 發票開立失敗 → 只寫 Log，不影響訂單流程
                                                        SaveErrorLog("TWPaymentCallback_Blessing_st 開立發票失敗 AdminID="
                                                ...
                                                {
                                                    // ❌ 個別筆數處理例外錯誤
                                                    SaveErrorLog("TWPaymentCallback_Blessing_st" + $"開立發票處理例外, 取得訂單錯誤。");
                                                }
                                            ...
                                            {
                                                // ❌ 個別筆數處理例外錯誤
                                                SaveErrorLog("TWPaymentCallback_Blessing_st" + $"開立發票處理例外 ex: " + ex.InnerException.Message);
                                            }
                                    ...
                                    {
                                        // ❌ 更新流水付費表資訊失敗，記錄錯誤
                                        SaveErrorLog("TWPaymentCallback_Blessing_st" + $"更新流水付費表資訊失敗");
                            ...
                            {
                                if (objLightDAC.UpdateChargeStatus_Blessing_st(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                                {
                                    // ❌ 此用戶已退款，記錄錯誤
                                    SaveErrorLog("TWPaymentCallback_Blessing_st" + $"此用戶已退款");
                                    Response.Write("<script>alert('此用戶已退款。');window.location.href='" + rebackURL + "'</script>");
                                }
                            }
                            else
                            {
                                objLightDAC.UpdateChargeStatus_Blessing_st(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
                                ...
                                {
                                    // ❌ 付款失敗，記錄錯誤
                                    SaveErrorLog("TWPaymentCallback_Blessing_st" + $"付款失敗，錯誤代碼：" + result[0]);
                        ...
                        {
                            //已經付費成功。
                            m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=" + kind + "&a=35&aid=" + aid +
                        ...
                        else if (status == -2)
                        {
                            // ❌ 此用戶已退款，記錄錯誤
                            SaveErrorLog("TWPaymentCallback_Blessing_st" + $"此用戶已退款");
                            Response.Write("<script>alert('此用戶已退款。');window.location.href='https://bobibobi.tw/Temples/templeInfo.aspx?a=35'</script>");
                        }
                        else
                        {
                            SaveErrorLog(resp + ", 此訂單已交易失敗!");
                            Response.Write("<script>alert('此訂單已交易失敗，交易代碼：" + resp + "如有疑問。請洽客服電話：04-36092299。');" +
                                "window.location.href='https://bobibobi.tw/Temples/templeInfo.aspx?a=35'</script>");
                        }
                    }
                    else
                    {
