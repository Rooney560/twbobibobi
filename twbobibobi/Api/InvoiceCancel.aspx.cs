/* ===================================================================================================
   專案名稱：twbobibobi
   檔案名稱：InvoiceCancel.aspx.cs
   類別說明：WebForms API 頁面，處理 /Api/InvoiceCancel.aspx POST 請求，實現發票作廢功能
   建立日期：2025-11-28
   建立人員：Rooney
...
{
    /// <summary>
    /// WebForms API 頁面：處理 /Api/InvoiceCancel.aspx POST 請求，提供發票作廢功能。
    /// </summary>
    /// <remarks>
    /// 此頁面負責處理來自 API 的發票作廢請求。它會接收請求，解析 JSON 內容，並調用發票作廢服務。
    /// 若作廢成功，會返回成功的結果；若有錯誤，則返回相應的錯誤訊息。
    /// </remarks>
    public partial class InvoiceCancel : AjaxBasePage
    {
        /// <summary>發票作廢服務</summary>
        IInvoiceCancelService _service = InvoiceServiceFactory.CreateCancelService(GetEnvironment());
        ...
        private void ProcessRequest()
        {
            ...
            try
            {
                ...
                invoiceNumber = obj?.Value<string>("CancelInvoiceNumber");
            }
            catch
            {
                // 解析 JSON 失敗，記錄錯誤
                basePage.SaveErrorLog("InvoiceCancel.ProcessRequest(無法解析 JSON 格式)");
            ...
            {
                basePage.SaveErrorLog("InvoiceCancel.ProcessRequest(請提供 CancelInvoiceNumber)");
                WriteJsonError(400, "請提供 CancelInvoiceNumber");
                return;
            }
            ...
            {
                // 呼叫發票作廢服務
                var raw = Task.Run(() => _service.CancelInvoiceAsync(invoiceNumber)).Result;
                var dto = JObject.Parse(raw).ToCancelInvoiceResponseDto(invoiceNumber);
            ...
            catch (Exception error)
            {
                ...
                basePage.SaveErrorLog("InvoiceCancel.ProcessRequest(伺服器內部錯誤)：\r\n" + detailedError);
