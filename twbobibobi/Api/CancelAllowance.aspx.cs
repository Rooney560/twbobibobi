/* ===================================================================================================
   專案名稱：twbobibobi
   檔案名稱：CancelAllowance.cs
   類別說明：提供折讓單作廢 API（WebForms API）
   建立日期：2025-12-17
   建立人員：Rooney
...
{
    /// <summary>
    /// /Api/CancelAllowance.aspx?allowanceNumber=XXX&env=YYY API 頁面  
    /// 功能：依據 allowanceNumber 作廢折讓單，並支援 UAT / Prod 環境切換、簽章驗證、防重放攻擊
    /// </summary>
    /// <remarks>
    /// 這個頁面處理折讓單作廢請求，包含驗證參數、簽章、時間戳以及折讓單狀態檢查，最後呼叫折讓單作廢 API 並回傳結果。
    /// </remarks>
    public partial class CancelAllowance : AjaxBasePage
    {
        ...
        IInvoiceStatusService _service_status = AllowanceServiceFactory.CreateStatusService(GetEnvironment());
        /// <summary>折讓單作廢建立服務</summary>
        IInvoiceCancelService _service_cancel = AllowanceServiceFactory.CreateCancelService(GetEnvironment());
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                // 讀參數
                string allowanceNo = Request["allowanceNumber"];
                ...
                // 1. 基本參數檢查
                if (string.IsNullOrWhiteSpace(allowanceNo) ||
                ...
                // 2. 驗證簽章
                if (!JSonHelper.ValidateSignature(env, ts, sig, allowanceNo, orderID, adminID, applicantID, kind))
                {
        ...
        /// 1. 取得相對應的購買人資料
        /// 2. 查詢折讓單狀態  
        /// 3. 驗證折讓單類型-D0401(折讓單開立)及折讓單狀態-99(已完成)
        /// 4. 呼叫廠商作廢 API  
        /// 5. 回傳 JSON  
        /// </summary>
        private void ProcessRequest()
        {
                ...
                !int.TryParse(kind, out kindInt) || kindInt <= 0)
            {
                ...
                basePage.SaveErrorLog("CancelAllowance.ProcessRequest：\r\n" + msg);
            ...
            // allowanceNo 仍然保留為 string，但可以在這裡對其進行額外的處理或驗證
            if (string.IsNullOrWhiteSpace(allowanceNo))
            {
                string msg = $"缺少或無效的參數：allowanceNumber:{allowanceNo}";
                basePage.SaveErrorLog("CancelAllowance.ProcessRequest：\r\n" + msg);
                WriteJsonError(400, msg);
                return;
            }
            ...
            try
            {
                ...
                if (allowanceRow["InvoiceNumber"].ToString() != allowanceNo)
                {
                    string msg = $"參數與訂單內的折讓單不匹配。";
                    basePage.SaveErrorLog("CancelAllowance.ProcessRequest：\r\n" + msg);
                    WriteJsonError(400, msg);
                    return;
                }
                ...
                var statusJson = Task.Run(() =>
                    _service_status.QueryStatusAsync(allowanceNo)).Result;
                var statusDto = JObject.Parse(statusJson).ToAllowanceStatusResponseDto();
                ...
                // 3. 必須是折讓單已開立 (D0401) 且折讓單狀態已完成 (99)
                if (statusDto.Data[0].Type != "D0401" || statusDto.Data[0].Status != "99")
                {
                    string msg = $"折讓單類型（{statusDto.Data[0].Type}）及折讓單狀態（{statusDto.Data[0].Status}）不允許作廢。";
                    basePage.SaveErrorLog("CancelAllowance.ProcessRequest：\r\n" + msg);
                    WriteJsonError(400, msg);
                    return;
                }
                ...
                var cancelJson = Task.Run(() =>
                    _service_cancel.CancelAllowanceAsync(allowanceNo)).Result;
                var cancelDto = JObject.Parse(cancelJson).ToCancelAllowanceResponseDto(allowanceNo);
            ...
            catch (Exception ex)
            {
                ...
                string detailedError = ErrorLogger.FormatError(ex, typeof(CancelAllowance).FullName);
                basePage.SaveErrorLog("CancelAllowance.ProcessRequest：\r\n" + detailedError);
