/* ===================================================================================================
   專案名稱：twbobibobi
   檔案名稱：InvoiceQuery.aspx.cs
   類別說明：WebForms API 頁面，處理 /Api/InvoiceQuery.aspx POST 請求，查詢發票資訊
   建立日期：2025-11-28
   建立人員：Rooney

 * 修改記錄：2025-12-18 新增發票號碼查詢條件。
   目前維護人員：Rooney
   =================================================================================================== */
...
{
    /// <summary>
    /// WebForms API 頁面：處理 /Api/InvoiceQuery.aspx POST 請求，提供查詢發票資訊的功能。
    /// </summary>
    /// <remarks>
    /// 這個頁面接收 POST 請求並根據提供的 orderID 或 invoiceNumber 查詢發票資訊。
    /// 當查詢成功時，會返回包含發票號碼、創建時間、隨機碼等資料的結果。
    ...
    public partial class InvoiceQuery : AjaxBasePage
    {
        ...
        /// <summary>
        /// 處理查詢發票資訊的請求：
        /// 1. 解析 JSON 並檢查是否有提供必要的參數（orderID 或 invoiceNumber）。
        /// 2. 呼叫發票查詢服務。
        /// 3. 回傳查詢結果或錯誤訊息。
        /// </summary>
        private void ProcessRequest()
        {
            ...
            {
                // 解析 JSON 失敗，記錄錯誤
                string detailedError = ErrorLogger.FormatError(error, typeof(InvoiceQuery).FullName);
            ...
            // 如果兩個參數都沒有，返回錯誤
            if (string.IsNullOrWhiteSpace(orderID) && string.IsNullOrWhiteSpace(invoiceNumber))
            {
                basePage.SaveErrorLog("InvoiceQuery.ProcessRequest(請提供 orderID 或 InvoiceNumber)");
                WriteJsonError(400, "請提供 orderID 或 InvoiceNumber");
                return;
            }
            ...
            try
            {
                ...
                {
                    // 如果提供了 InvoiceNumber，則優先使用發票號碼查詢
                    raw = Task.Run(() => _service.QueryInvoiceNumberAsync(invoiceNumber)).Result;
                }
                else
                {
                    // 否則使用 orderID 查詢
                    raw = Task.Run(() => _service.QueryOrderIdAsync(orderID)).Result;
                }
