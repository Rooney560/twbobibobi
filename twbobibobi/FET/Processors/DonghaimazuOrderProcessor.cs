...
namespace twbobibobi.FET.Processors
{
    ...
    /// 實作 ITempleOrderProcessor，台東東海龍門天聖宮建單邏輯
    /// </summary>
    public class DonghaimazuOrderProcessor : TempleOrderProcessorBase
    {
        /// <summary> 宮廟代碼：台東東海龍門天聖宮 (AdminID = 16) </summary>
        private readonly int _adminID = 16;

        /// <summary> API 專用後綴來源網址，用於串接後端流程。 </summary>
        private string PostUrl = "_dh_Index_FETAPI";

        /// <summary>
        /// 建構式：建立處理器實例-台東東海龍門天聖宮。
        /// </summary>
        /// <param name="page">目前頁面 BasePage 實例，用於記錄與取得 HttpContext。</param>
        /// <param name="year">當前年度字串。</param>
        public DonghaimazuOrderProcessor(
            BasePage page,
            string year)
            : base(page, year)
        {
            ...
            string OrderId)
        {
            ...
            switch (kind)
            {
                ...
                case "1":  
                    ...
                    // 新增購買人資料
                    int applicantId = _lightDAC.Addapplicantinfo_lights_dh(
                ...
                case "2":
                    ...
                    // 新增購買人資料
                    applicantId = _lightDAC.Addapplicantinfo_purdue_dh(
                        Name: applicant.appName,
                        Mobile: applicant.appMobile,
                        Cost: totalAmount,
                        County: applicant.appCity,
                                             ...
                                             DateTime dtNow)
        {
            ...
            foreach (var p in prayedPersons)
            {
                ...
                // 呼叫 LightDAC
                int lightsId = _lightDAC.AddLights_dh(
            ...
            _lightDAC.AddChargeLog_Lights_dh(
                OrderID: OrderId,
                ApplicantID: applicantId,
                Amount: totalAmount,
                ChargeType: "FETAPI",
            ...
            if (dtCharge != null && dtCharge.Rows.Count > 0 && Convert.ToInt32(dtCharge.Rows[0]["Status"]) == 0)
            {
                ...
                int uStatus = 0;
                //更新流水付費表資訊(付費成功)
                if (!_lightDAC.UpdateChargeLog_Lights_dh(
                    OrderId,
                    Tid,
                    msg,
                                             ...
                                             DateTime dtNow)
        {
            ...
            foreach (var p in prayedPersons)
            {
                ...
                if (ddod.Length == 7 && dbirth.Length == 7)
                {
                    ...
                    // 如果紀元為民國前
                    string syear = p.dBirthType == "1" ? drocSy.ToString() : "前" + drocSy.ToString();

                    // 國曆格式
                    dDoD = $"民國{drocSy}年{ddaySy:00}月{ddaySy:00}日";
                ...
                // 呼叫 LightDAC
                int purdueId = _lightDAC.Addpurdue_dh(
                    ApplicantID: applicantId,
                    Name: p.Name,
                    Mobile: mobile,
                    Cost: p.Cost,
                    ...
                    DeathBirthMonth: p.dBirthMonth,
                    DeathAge: p.dAge,
                    DeathZodiac: p.dZodiac,
            ...
            // 建立交易流水
            string callbackLog = $"{Tid},{fetOrderNumber}";
            _lightDAC.AddChargeLog_Purdue_dh(
                OrderID: OrderId,
                ApplicantID: applicantId,
                Amount: totalAmount,
                ChargeType: "FETAPI",
            ...
            // 更新並獲取燈號
            DataTable dtCharge = _lightDAC.GetChargeLog_Purdue_dh(OrderId, _year);
            if (dtCharge != null && dtCharge.Rows.Count > 0 && Convert.ToInt32(dtCharge.Rows[0]["Status"]) == 0)
            {
                int purdueType = _lightDAC.GetPurdueType_dh(applicantId, _year);

                string msg = "【保必保庇】線上宮廟服務平臺，感謝購買，已成功付款" + totalAmount + "元，您的訂單編號 ";

                var purdueArr = new string[purdueList.Length];
                _lightDAC.UpdatePurdue_dh_Info(
                    applicantID: applicantId,
                    Year: _year,
                    ref msg,
                    ref purdueList,
                ...
                int uStatus = 0;
                //更新流水付費表資訊(付費成功)
                if (!_lightDAC.UpdateChargeLog_Purdue_dh(
                    OrderId,
                    Tid,
                    msg,
                    _page.Request.UserHostAddress,
