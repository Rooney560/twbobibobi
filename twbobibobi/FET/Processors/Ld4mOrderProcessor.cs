...
{
    /// <summary>
    /// Class: Ld4mOrderProcessor
    /// 實作 <see cref="ITempleOrderProcessor"/>，
    /// 負責處理「桃園龍德宮」的訂單邏輯（點燈 / 普度）。
    /// </summary>
    public class Ld4mOrderProcessor : TempleOrderProcessorBase
    {
        /// <summary> 宮廟代碼：桃園龍德宮 (AdminID = 32) </summary>
        private readonly int _adminID = 32;

        /// <summary> API 專用後綴來源網址，用於串接後端流程。 </summary>
        private string PostUrl = "_ld_Index_FETAPI";
        ...
        /// <param name="kind">服務種類代碼 (1=點燈, 2=普度)。</param>
        ...
        protected override Task<List<string>> HandleOrderAsync(
            ...
            string OrderId)
        {
            // 進桃園龍德宮的處理邏輯時，先記錄一筆日誌
            var page = HttpContext.Current.Handler as CreateOrder;
            if (page != null)
            {
                page.SaveTimingLog("進入 Ld4mOrderProcessor.HandleOrderAsync", 0);
            }
            ...
            switch (kind)
            {
                ...
                case "2":
                    ...
                    // 新增購買人資料
                    applicantId = _lightDAC.Addapplicantinfo_purdue_ld(
                        Name: applicant.appName,
                        Mobile: applicant.appMobile,
                        Cost: totalAmount,
                                             ...
                                             DateTime dtNow)
        {
            ...
            foreach (var p in prayedPersons)
            {
                ...
                // 呼叫 LightDAC
                int lightsId = _lightDAC.AddLights_ld(
            ...
            _lightDAC.AddChargeLog_Lights_ld(
                OrderID: OrderId,
                ApplicantID: applicantId,
                Amount: totalAmount,
                ChargeType: "FETAPI",
            ...
            if (dtCharge != null && dtCharge.Rows.Count > 0 && Convert.ToInt32(dtCharge.Rows[0]["Status"]) == 0)
            {
                ...
                int uStatus = 0;
                //更新流水付費表資訊(付費成功)
                if (!_lightDAC.UpdateChargeLog_Lights_ld(
                    OrderId,
                    Tid,
                    msg,
                                             ...
                                             DateTime dtNow)
        {
            ...
            foreach (var p in prayedPersons)
            {
                ...
                // 加購普品數量
                int.TryParse(p.AdditionalOfferingQty, out int count);
                ...
                switch (PurdueType)
                {
                    ...
                    case "11":
                        ...
                        // 寵物 國曆死亡日期，並計算死亡時年齡
                        string pDeath = p.petDeath;
                        if (pDeath.Length == 7)
                        {
                            int drocSy = int.Parse(pDeath.Substring(0, 3));
                            int dyearSyAD = drocSy + 1911;
                            int dmonthSy = int.Parse(pDeath.Substring(3, 2));
                            int ddaySy = int.Parse(pDeath.Substring(5, 2));

                            // 國曆格式
                            p.petDeathday = $"民國{drocSy}年{dmonthSy:00}月{ddaySy:00}日";
                        }
                        else
                        {
                            p.petDeathday = p.petDeath;
                        }
                        break;
                }

                // 呼叫 LightDAC
                int purdueId = _lightDAC.Addpurdue_ld(
                    ApplicantID: applicantId,
                    Name: p.Name,
                    ...
                    DeathName: p.dName ?? "",
                    DeathBirth: p.dBirth ?? "",
                    DeathLeapMonth: p.dLeapMonth ?? "N",
                    DeathBirthTime: p.dBirthTime ?? "吉",
                    Deathday: p.dDoD ?? "",
                    DeathAge: p.dAge ?? "0",
                    DeathAddr: p.dAddr ?? "",
                    DeathCounty: p.dCity ?? "",
                    ...
                    PetName: p.PetName ?? "",
                    PetType: p.PetType ?? "",
                    Petdeathday: p.petDeathday ?? "",
                    Year: _year);
            }

            // 建立交易流水
            string callbackLog = $"{Tid},{fetOrderNumber}";
            _lightDAC.AddChargeLog_Purdue_ld(
                OrderID: OrderId,
                ApplicantID: applicantId,
                Amount: totalAmount,
                ChargeType: "FETAPI",
            ...
            // 更新並獲取燈號
            DataTable dtCharge = _lightDAC.GetChargeLog_Purdue_ld(OrderId, _year);
            if (dtCharge != null && dtCharge.Rows.Count > 0 && Convert.ToInt32(dtCharge.Rows[0]["Status"]) == 0)
            {
                int purdueType = _lightDAC.GetPurdueType_ld(applicantId, _year);

                string msg = "感謝購買,已成功付款" + totalAmount + "元,您的訂單編號 ";

                var purdueArr = new string[purdueList.Length];
                _lightDAC.UpdatePurdue_ld_Info(
                    applicantID: applicantId,
                    Year: _year,
                    ref msg,
                    ref purdueList,
                ...
                int uStatus = 0;
                //更新流水付費表資訊(付費成功)
                if (!_lightDAC.UpdateChargeLog_Purdue_ld(
                    OrderId,
                    Tid,
                    msg,
                    _page.Request.UserHostAddress,
