...
namespace twbobibobi.FET.Processors
{
    ...
    /// 實作 ITempleOrderProcessor，西螺福興宮建單邏輯
    /// </summary>
    public class FusingOrderProcessor : TempleOrderProcessorBase
    {
        /// <summary> 宮廟代碼：西螺福興宮 (AdminID = 8) </summary>
        private readonly int _adminID = 8;

        /// <summary> API 專用後綴來源網址，用於串接後端流程。 </summary>
        private string PostUrl = "_Fu_Index_FETAPI";
            ...
            string OrderId)
        {
            ...
            switch (kind)
            {
                ...
                case "1":  
                    ...
                    // 新增購買人資料
                    int applicantId = _lightDAC.Addapplicantinfo_lights_Fu(
                ...
                case "2":
                    ...
                    // 新增購買人資料
                    applicantId = _lightDAC.Addapplicantinfo_purdue_Fu(
                        Name: applicant.appName,
                        Mobile: applicant.appMobile,
                        Cost: totalAmount,
                        County: applicant.appCity,
                                             ...
                                             DateTime dtNow)
        {
            ...
            foreach (var p in prayedPersons)
            {
                ...
                // 呼叫 LightDAC
                int lightsId = _lightDAC.AddLights_Fu(
            ...
            _lightDAC.AddChargeLog_Lights_Fu(
                OrderID: OrderId,
                ApplicantID: applicantId,
                Amount: totalAmount,
                ChargeType: "FETAPI",
            ...
            if (dtCharge != null && dtCharge.Rows.Count > 0 && Convert.ToInt32(dtCharge.Rows[0]["Status"]) == 0)
            {
                ...
                int uStatus = 0;
                //更新流水付費表資訊(付費成功)
                if (!_lightDAC.UpdateChargeLog_Lights_Fu(
                    OrderId,
                    Tid,
                    msg,
                                             ...
                                             DateTime dtNow)
        {
            ...
            foreach (var p in prayedPersons)
            {
                ...
                // 加購金紙數量
                int.TryParse(p.JossMoneyQty, out int Glodcount);
                ...
                // 呼叫 LightDAC
                int purdueId = _lightDAC.Addpurdue_Fu(
                    ApplicantID: applicantId,
                    Name: p.Name,
            ...
            // 建立交易流水
            string callbackLog = $"{Tid},{fetOrderNumber}";
            _lightDAC.AddChargeLog_Purdue_Fu(
                OrderID: OrderId,
                ApplicantID: applicantId,
                Amount: totalAmount,
                ChargeType: "FETAPI",
            ...
            // 更新並獲取燈號
            DataTable dtCharge = _lightDAC.GetChargeLog_Purdue_Fu(OrderId, _year);
            if (dtCharge != null && dtCharge.Rows.Count > 0 && Convert.ToInt32(dtCharge.Rows[0]["Status"]) == 0)
            {
                int purdueType = _lightDAC.GetPurdueType_Fu(applicantId, _year);

                string msg = "感謝購買,已成功付款" + totalAmount + "元,您的訂單編號 ";

                var purdueArr = new string[purdueList.Length];
                _lightDAC.UpdatePurdue_Fu_Info(
                    applicantID: applicantId,
                    Year: _year,
                    ref msg,
                    ref purdueList,
                ...
                int uStatus = 0;
                //更新流水付費表資訊(付費成功)
                if (!_lightDAC.UpdateChargeLog_Purdue_Fu(
                    OrderId,
                    Tid,
                    msg,
                    _page.Request.UserHostAddress,
