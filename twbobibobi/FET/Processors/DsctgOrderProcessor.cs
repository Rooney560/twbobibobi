...
{
    /// <summary>
    /// Class: DsctgOrderProcessor
    /// 實作 ITempleOrderProcessor，玉敕大樹朝天宮建單邏輯
    /// </summary>
    public class DsctgOrderProcessor : TempleOrderProcessorBase
    {
        /// <summary> 宮廟代碼：玉敕大樹朝天宮 (AdminID = 23) </summary>
        private readonly int _adminID = 23;

        /// <summary> API 專用後綴來源網址，用於串接後端流程。 </summary>
        private string PostUrl = "_ma_Index_FETAPI";
            ...
            string OrderId)
        {
            ...
            switch (kind)
            {
                ...
                case "1":  
                    ...
                    // 新增購買人資料
                    int applicantId = _lightDAC.Addapplicantinfo_lights_ma(
                ...
                case "2":
                    ...
                    // 新增購買人資料
                    applicantId = _lightDAC.Addapplicantinfo_purdue_ma(
                        Name: applicant.appName,
                        Mobile: applicant.appMobile,
                        Cost: totalAmount,
                        County: applicant.appCity,
                ...
                case "7":
                    ...
                    // 新增購買人資料
                    applicantId = _lightDAC.Addapplicantinfo_supplies_ma(
                        Name: applicant.appName,
                        Mobile: applicant.appMobile,
                        Cost: totalAmount,
                        County: applicant.appCity,
                ...
                // 靈寶禮斗
                case "12":
                    PostUrl = "Lingbaolidou" + PostUrl;

                    // 新增購買人資料
                    applicantId = _lightDAC.Addapplicantinfo_Lingbaolidou_ma(
                        Name: applicant.appName,
                        Mobile: applicant.appMobile,
                        Email: applicant.appEmail,
                        Cost: totalAmount,
                    ...
                    resultOrderNumbers.AddRange(ProcessLingbaolidouing(applicantId, applicant, prayedPersons, Tid, fetOrderNumber, amount, itemsInfo, OrderId, dtNow));
                    break;
                                             ...
                                             DateTime dtNow)
        {
            ...
            foreach (var p in prayedPersons)
            {
                ...
                // 呼叫 LightDAC
                int lightsId = _lightDAC.AddLights_ma(
            ...
            _lightDAC.AddChargeLog_Lights_ma(
                OrderID: OrderId,
                ApplicantID: applicantId,
                Amount: totalAmount,
                ChargeType: "FETAPI",
            ...
            if (dtCharge != null && dtCharge.Rows.Count > 0 && Convert.ToInt32(dtCharge.Rows[0]["Status"]) == 0)
            {
                ...
                int uStatus = 0;
                //更新流水付費表資訊(付費成功)
                if (!_lightDAC.UpdateChargeLog_Lights_ma(
                    OrderId,
                    Tid,
                    msg,
                                             ...
                                             DateTime dtNow)
        {
            ...
            foreach (var p in prayedPersons)
            {
                ...
                // 呼叫 LightDAC
                int purdueId = _lightDAC.Addpurdue_ma(
                    ApplicantID: applicantId,
                    Name: p.Name,
                    Mobile: mobile,
                    Cost: p.Cost,
            ...
            // 建立交易流水
            string callbackLog = $"{Tid},{fetOrderNumber}";
            _lightDAC.AddChargeLog_Purdue_ma(
                OrderID: OrderId,
                ApplicantID: applicantId,
                Amount: totalAmount,
                ChargeType: "FETAPI",
            ...
            // 更新並獲取燈號
            DataTable dtCharge = _lightDAC.GetChargeLog_Purdue_ma(OrderId, _year);
            if (dtCharge != null && dtCharge.Rows.Count > 0 && Convert.ToInt32(dtCharge.Rows[0]["Status"]) == 0)
            {
                int purdueType = _lightDAC.GetPurdueType_ma(applicantId, _year);

                string msg = "【保必保庇】線上宮廟服務平臺，感謝購買，已成功付款" + totalAmount + "元，您的訂單編號 ";

                var purdueArr = new string[purdueList.Length];
                _lightDAC.UpdatePurdue_ma_Info(
                    applicantID: applicantId,
                    Year: _year,
                    ref msg,
                    ref purdueList,
                ...
                int uStatus = 0;
                //更新流水付費表資訊(付費成功)
                if (!_lightDAC.UpdateChargeLog_Purdue_ma(
                    OrderId,
                    Tid,
                    msg,
                    _page.Request.UserHostAddress,
                                             ...
                                             DateTime dtNow)
        {
            ...
            foreach (var p in prayedPersons)
            {
                ...
                // 呼叫 LightDAC
                int suppliesId = _lightDAC.Addsupplies_ma(
                    ApplicantID: applicantId,
                    Name: p.Name,
                    Mobile: mobile,
                    Cost: p.Cost,
            ...
            // 建立交易流水
            string callbackLog = $"{Tid},{fetOrderNumber}";
            _lightDAC.AddChargeLog_Supplies_ma(
                OrderID: OrderId,
                ApplicantID: applicantId,
                Amount: totalAmount,
                ChargeType: "FETAPI",
            ...
            // 更新並獲取燈號
            DataTable dtCharge = _lightDAC.GetChargeLog_Supplies_ma(OrderId, _year);
            if (dtCharge != null && dtCharge.Rows.Count > 0 && Convert.ToInt32(dtCharge.Rows[0]["Status"]) == 0)
            {
                string msg = "【保必保庇】線上宮廟服務平臺，感謝購買，已成功付款" + totalAmount + "元，您的訂單編號 ";

                var suppliesArr = new string[suppliesList.Length];
                _lightDAC.UpdateSupplies_ma_Info(
                    applicantID: applicantId,
                    Year: _year,
                    ref msg,
                    ref suppliesList,
                ...
                int uStatus = 0;
                //更新流水付費表資訊(付費成功)
                if (!_lightDAC.UpdateChargeLog_Supplies_ma(
                    OrderId,
                    Tid,
                    msg,
                    _page.Request.UserHostAddress,
        ...
        // 靈寶禮斗
        private List<string> ProcessLingbaolidouing(int applicantId,
                                             ApplicantDto applicant,
                                             List<PrayedPersonDto> prayedPersons,
                                             string Tid,
                                             string fetOrderNumber,
                                             ...
                                             DateTime dtNow)
        {
            ...
            foreach (var p in prayedPersons)
            {
                ...
                // 呼叫 LightDAC
                int LingbaolidouID = _lightDAC.AddLingbaolidou_ma(
                    ApplicantID: applicantId,
                    Name: p.Name,
                    Mobile: mobile,
                    Cost: p.Cost,
                    Sex: gender,
                    LingbaolidouType: p.TypeID,
                    LingbaolidouString: p.TypeString,
                    Oversea: oversea,
                    Birth: p.Birth,
                    LeapMonth: p.LeapMonth,
                    BirthTime: p.BirthTime,
            ...
            // 建立交易流水
            string callbackLog = $"{Tid},{fetOrderNumber}";
            _lightDAC.AddChargeLog_Lingbaolidou_ma(
                OrderID: OrderId,
                ApplicantID: applicantId,
                Amount: totalAmount,
                ChargeType: "FETAPI",
            ...
            // 更新並獲取燈號
            DataTable dtCharge = _lightDAC.GetChargeLog_Lingbaolidou_ma(OrderId, _year);
            if (dtCharge != null && dtCharge.Rows.Count > 0 && Convert.ToInt32(dtCharge.Rows[0]["Status"]) == 0)
            {
                string msg = "【保必保庇】線上宮廟服務平臺，感謝購買，已成功付款" + totalAmount + "元，您的訂單編號 ";

                var LingbaolidouArr = new string[LingbaolidouList.Length];
                _lightDAC.UpdateLingbaolidou_ma_Info(
                    applicantID: applicantId,
                    Year: _year,
                    ref msg,
                    ref LingbaolidouList,
                    ref LingbaolidouArr);
                orderNumbers.AddRange(LingbaolidouArr);

                string ChargeType = string.Empty;
                int uStatus = 0;
                //更新流水付費表資訊(付費成功)
                if (!_lightDAC.UpdateChargeLog_Lingbaolidou_ma(
                    OrderId,
                    Tid,
                    msg,
                    _page.Request.UserHostAddress,
