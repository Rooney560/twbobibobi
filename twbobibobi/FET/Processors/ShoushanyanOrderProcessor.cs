...
namespace twbobibobi.FET.Processors
{
    ...
    /// 實作 ITempleOrderProcessor，壽山巖觀音寺建單邏輯
    /// </summary>
    public class ShoushanyanOrderProcessor : TempleOrderProcessorBase
    {
        /// <summary> 宮廟代碼：壽山巖觀音寺 (AdminID = 41) </summary>
        private readonly int _adminID = 41;

        /// <summary> API 專用後綴來源網址，用於串接後端流程。 </summary>
        private string PostUrl = "_ssy_Index_FETAPI";
        ...
        public ShoushanyanOrderProcessor(
            BasePage page,
            string year)
            : base(page, year)
        {
        ...
        protected override Task<List<string>> HandleOrderAsync(
            ...
            string OrderId)
        {
            // 進壽山巖觀音寺的處理邏輯時，先記錄一筆日誌
            var page = HttpContext.Current.Handler as CreateOrder;
            if (page != null)
            {
                page.SaveTimingLog("進入 ShoushanyanOrderProcessor.HandleOrderAsync", 0);
            }
            ...
            switch (kind)
            {
                ...
                case "1":
                    ...
                    // 新增購買人資料
                    int applicantId = _lightDAC.Addapplicantinfo_lights_ssy(
                                             ...
                                             DateTime dtNow)
        {
            ...
            foreach (var p in prayedPersons)
            {
                ...
                // 呼叫 LightDAC
                int lightsId = _lightDAC.AddLights_ssy(
            ...
            _lightDAC.AddChargeLog_Lights_ssy(
                OrderID: OrderId,
                ApplicantID: applicantId,
                Amount: totalAmount,
                ChargeType: "FETAPI",
            ...
            if (dtCharge != null && dtCharge.Rows.Count > 0 && Convert.ToInt32(dtCharge.Rows[0]["Status"]) == 0)
            {
                ...
                int uStatus = 0;
                //更新流水付費表資訊(付費成功)
                if (!_lightDAC.UpdateChargeLog_Lights_ssy(
                    OrderId,
                    Tid,
                    msg,
