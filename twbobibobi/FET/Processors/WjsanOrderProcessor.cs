...
{
    /// <summary>
    /// Class: WjsanOrderProcessor
    /// 實作 ITempleOrderProcessor，台灣道教總廟無極三清總道院建單邏輯
    /// </summary>
    public class WjsanOrderProcessor : TempleOrderProcessorBase
    {
        /// <summary> 宮廟代碼：台灣道教總廟無極三清總道院 (AdminID = 31) </summary>
        private readonly int _adminID = 31;

        /// <summary> API 專用後綴來源網址，用於串接後端流程。 </summary>
        private string PostUrl = "_wjsan_Index_FETAPI";

        /// <summary>
        /// 建構式：建立處理器實例-台灣道教總廟無極三清總道院。
        /// </summary>
        /// <param name="page">目前頁面 BasePage 實例，用於記錄與取得 HttpContext。</param>
        /// <param name="year">當前年度字串。</param>
        public WjsanOrderProcessor(
            BasePage page,
            string year)
            : base(page, year)
        {
            ...
            string OrderId)
        {
            ...
            switch (kind)
            {
                ...
                case "1":  
                    ...
                    // 新增購買人資料
                    int applicantId = _lightDAC.Addapplicantinfo_lights_wjsan(
                ...
                case "2":
                    ...
                    // 新增購買人資料
                    applicantId = _lightDAC.Addapplicantinfo_purdue_wjsan(
                        Name: applicant.appName,
                        Mobile: applicant.appMobile,
                        Cost: totalAmount,
                        County: applicant.appCity,
                ...
                case "20":
                    ...
                    // 新增購買人資料
                    applicantId = _lightDAC.Addapplicantinfo_andou_wjsan(
                        Name: applicant.appName,
                        Mobile: applicant.appMobile,
                        Cost: totalAmount,
                        County: applicant.appCity,
                ...
                // 供花供果服務
                case "21":
                    PostUrl = "Huaguo" + PostUrl;

                    // 新增購買人資料
                    applicantId = _lightDAC.Addapplicantinfo_huaguo_wjsan (
                        Name: applicant.appName,
                        Mobile: applicant.appMobile,
                        Cost: totalAmount,
                        County: applicant.appCity,
                    ...
                    resultOrderNumbers.AddRange(ProcessHuaguoing(applicantId, applicant, prayedPersons, Tid, fetOrderNumber, amount, itemsInfo, OrderId, dtNow));
                    break;
                                             ...
                                             DateTime dtNow)
        {
            ...
            foreach (var p in prayedPersons)
            {
                ...
                // 呼叫 LightDAC
                int lightsId = _lightDAC.AddLights_wjsan(
            ...
            _lightDAC.AddChargeLog_Lights_wjsan(
                OrderID: OrderId,
                ApplicantID: applicantId,
                Amount: totalAmount,
                ChargeType: "FETAPI",
            ...
            // 更新並獲取燈號
            DataTable dtCharge = _lightDAC.GetChargeLog_Lights_wjsan(OrderId, _year);
            if (dtCharge != null && dtCharge.Rows.Count > 0 && Convert.ToInt32(dtCharge.Rows[0]["Status"]) == 0)
            {
                ...
                int uStatus = 0;
                //更新流水付費表資訊(付費成功)
                if (!_lightDAC.UpdateChargeLog_Lights_wjsan(
                    OrderId,
                    Tid,
                    msg,
                                             ...
                                             DateTime dtNow)
        {
            ...
            foreach (var p in prayedPersons)
            {
                ...
                // 呼叫 LightDAC
                int purdueId = _lightDAC.Addpurdue_wjsan(
                    ApplicantID: applicantId,
                    Name: p.Name,
                    Mobile: mobile,
                    Cost: p.Cost,
            ...
            // 建立交易流水
            string callbackLog = $"{Tid},{fetOrderNumber}";
            _lightDAC.AddChargeLog_Purdue_wjsan(
                OrderID: OrderId,
                ApplicantID: applicantId,
                Amount: totalAmount,
                ChargeType: "FETAPI",
            ...
            // 更新並獲取燈號
            DataTable dtCharge = _lightDAC.GetChargeLog_Purdue_wjsan(OrderId, _year);
            if (dtCharge != null && dtCharge.Rows.Count > 0 && Convert.ToInt32(dtCharge.Rows[0]["Status"]) == 0)
            {
                ...
                int uStatus = 0;
                //更新流水付費表資訊(付費成功)
                if (!_lightDAC.UpdateChargeLog_Purdue_wjsan(
                    OrderId,
                    Tid,
                    msg,
                    _page.Request.UserHostAddress,
                                             ...
                                             DateTime dtNow)
        {
            ...
            foreach (var p in prayedPersons)
            {
                ...
                // 呼叫 LightDAC
                int andouId = _lightDAC.AddAnDou_wjsan(
                    ApplicantID: applicantId,
                    Name: p.Name,
                    Mobile: mobile,
                    Cost: p.Cost,
            ...
            // 建立交易流水
            string callbackLog = $"{Tid},{fetOrderNumber}";
            _lightDAC.AddChargeLog_AnDou_wjsan(
                OrderID: OrderId,
                ApplicantID: applicantId,
                Amount: totalAmount,
                ChargeType: "FETAPI",
            ...
            // 更新並獲取燈號
            DataTable dtCharge = _lightDAC.GetChargeLog_AnDou_wjsan(OrderId, _year);
            if (dtCharge != null && dtCharge.Rows.Count > 0 && Convert.ToInt32(dtCharge.Rows[0]["Status"]) == 0)
            {
                ...
                int uStatus = 0;
                //更新流水付費表資訊(付費成功)
                if (!_lightDAC.UpdateChargeLog_AnDou_wjsan(
                    OrderId,
                    Tid,
                    msg,
                    _page.Request.UserHostAddress,
        ...
        // 供花供果服務
        private List<string> ProcessHuaguoing(int applicantId,
                                             ApplicantDto applicant,
                                             List<PrayedPersonDto> prayedPersons,
                                             string Tid,
                                             string fetOrderNumber,
                                             ...
                                             DateTime dtNow)
        {
            ...
            foreach (var p in prayedPersons)
            {
                ...
                // 呼叫 LightDAC
                int andouId = _lightDAC.AddHuaguo_wjsan(
                    ApplicantID: applicantId,
                    Name: p.Name,
                    Mobile: mobile,
                    Cost: p.Cost,
            ...
            // 建立交易流水
            string callbackLog = $"{Tid},{fetOrderNumber}";
            _lightDAC.AddChargeLog_Huaguo_wjsan(
                OrderID: OrderId,
                ApplicantID: applicantId,
                Amount: totalAmount,
                ChargeType: "FETAPI",
            ...
            // 更新並獲取燈號
            DataTable dtCharge = _lightDAC.GetChargeLog_Huaguo_wjsan(OrderId, _year);
            if (dtCharge != null && dtCharge.Rows.Count > 0 && Convert.ToInt32(dtCharge.Rows[0]["Status"]) == 0)
            {
                ...
                int uStatus = 0;
                //更新流水付費表資訊(付費成功)
                if (!_lightDAC.UpdateChargeLog_Huaguo_wjsan(
                    OrderId,
                    Tid,
                    msg,
                    _page.Request.UserHostAddress,
