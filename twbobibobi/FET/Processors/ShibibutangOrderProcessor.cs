...
{
    /// <summary>
    /// Class: ShibibutangOrderProcessor
    /// 實作 ITempleOrderProcessor，慈惠石壁部堂點燈建單邏輯
    /// </summary>
    public class ShibibutangOrderProcessor : TempleOrderProcessorBase
    {
        private readonly int _adminID = 39;
        private string PostUrl = "_sbbt_Index_FETAPI";
        ...
        public ShibibutangOrderProcessor(
            BasePage page,
            string year)
            : base(page, year)
        {
        ...
        protected override Task<List<string>> HandleOrderAsync(
            ...
            string OrderId)
        {
            // 進慈惠石壁部堂的處理邏輯時，先記錄一筆日誌
            var page = HttpContext.Current.Handler as CreateOrder;
            if (page != null)
            {
                page.SaveTimingLog("進入 ShibibutangOrderProcessor.HandleOrderAsync", 0);
            }
            ...
            switch (kind)
            {
                ...
                case "1":
                    ...
                    // 新增購買人資料
                    int applicantId = _lightDAC.Addapplicantinfo_lights_sbbt(
                                             ...
                                             DateTime dtNow)
        {
            ...
            foreach (var p in prayedPersons)
            {
                ...
                // 呼叫 LightDAC
                int lightsId = _lightDAC.AddLights_sbbt(
            ...
            _lightDAC.AddChargeLog_Lights_sbbt(
                OrderID: OrderId,
                ApplicantID: applicantId,
                Amount: totalAmount,
                ChargeType: "FETAPI",
            ...
            if (dtCharge != null && dtCharge.Rows.Count > 0 && Convert.ToInt32(dtCharge.Rows[0]["Status"]) == 0)
            {
                ...
                int uStatus = 0;
                //更新流水付費表資訊(付費成功)
                if (!_lightDAC.UpdateChargeLog_Lights_sbbt(
                    OrderId,
                    Tid,
                    msg,
