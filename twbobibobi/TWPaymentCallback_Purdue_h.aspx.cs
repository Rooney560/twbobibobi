        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                ...
                if (dtCharge.Rows.Count > 0)
                {
                    ...
                    if (status == 0)
                    {
                        ...
                        if (result[0] == "1" || result[0] == "2")
                        {
                            if (result.Length > 1)
                            {
                                ...
                                //取得申請人資料表
                                //DataTable dtapplicantinfo = objDatabaseHelper.Getapplicantinfo_Purdue_h(aid, adminID, Year);
                                ////更新購買表內購買人狀態為已付款(Status=2)
                                //int cost = dtapplicantinfo.Rows.Count > 0 ? int.Parse(dtapplicantinfo.Rows[0]["Cost"].ToString()) : 0;
                                objDatabaseHelper.Updateapplicantinfo_Purdue_h(aid, cost, 2, Year);
                                ...
                                ////更新普渡資料表並取得訂單編號
                                //objDatabaseHelper.UpdatePurdue_h_Info(aid, ref purduelist);
                                ////取得申請人資料表
                                //DataTable dtapplicantinfo = objDatabaseHelper.Getapplicantinfo_Purdue_h(aid, adminID);
                                ////更新購買表內購買人狀態為已付款(Status=2)
                                //objDatabaseHelper.Updateapplicantinfo_Purdue_h(aid);
                                ...
                                ////更新流水付費表資訊(付費成功)
                                //if (objDatabaseHelper.UpdateChargeLog_Purdue_h(orderId, tid, msg, Request.UserHostAddress, CallbackLog))
                                //{
                                //    if (objSMSHepler.SendMsg_SL(mobile, msg))
                                //    {
                        ...
                        {
                            if (objDatabaseHelper.UpdateChargeStatus_Purdue_h(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                            {
                        ...
                        {
                            objDatabaseHelper.UpdateChargeStatus_Purdue_h(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
                    ...
                    {
                        //已經付費成功。
                        m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=2&a=4&aid=" + aid + (dtCharge.Rows[0]["ChargeType"].ToString() == "Twm" ? "&twm=1" : "");
                        Response.Redirect(m2, true);
                    }
                    else
                    {
