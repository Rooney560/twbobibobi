    ...
    public partial class TWPaymentCallback_Supplies3_ty : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                ...
                int status = 999;
                int adminID = objDatabaseHelper.GetAdminID_Supplies3_ty(aid, Year);
                ...
                if (dtCharge.Rows.Count > 0)
                {
                    ...
                    if (status == 0)
                    {
                        ...
                        if (result[0] == "1" || result[0] == "2")
                        {
                            ...
                            objDatabaseHelper.Updateapplicantinfo_Supplies3_ty(aid, cost, 2, Year); //更新購買表內購買人狀態為已付款(Status=2)
                            ...
                            if (objDatabaseHelper.UpdateChargeLog_Supplies3_ty(orderId, tid, msg, Request.UserHostAddress, CallbackLog, Year, ref ChargeType))
                            {
                                ...
                                {
                                    m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?a=" + adminID + "&aid=" + aid + "&kind=18" +
                        ...
                        {
                            if (objDatabaseHelper.UpdateChargeStatus_Supplies3_ty(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                            {
                        ...
                        {
                            objDatabaseHelper.UpdateChargeStatus_Supplies3_ty(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
