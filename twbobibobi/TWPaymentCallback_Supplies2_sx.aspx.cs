    ...
    public partial class TWPaymentCallback_Supplies2_sx : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                ...
                DataTable dtCharge = objDatabaseHelper.GetChargeLog_Supplies2_sx(orderId, Year);
                ...
                if (dtCharge.Rows.Count > 0)
                {
                    ...
                    if (status == 0)
                    {
                        ...
                        if (result[0] == "1" || result[0] == "2")
                        {
                            if (result.Length > 1)
                            {
                                ...
                                //更新普渡資料表並取得訂單編號
                                objDatabaseHelper.UpdateSupplies2_sx_Info(aid, Year, ref msg, ref supplieslist);
                                //取得申請人資料表
                                //DataTable dtapplicantinfo = objDatabaseHelper.Getapplicantinfo_Supplies2_sx(aid, adminID, Year);
                                ////更新購買表內購買人狀態為已付款(Status=2)
                                //int cost = dtapplicantinfo.Rows.Count > 0 ? int.Parse(dtapplicantinfo.Rows[0]["Cost"].ToString()) : 0;
                                objDatabaseHelper.Updateapplicantinfo_Supplies2_sx(aid, cost, 2, Year);
                                ...
                                //更新流水付費表資訊(付費成功)
                                if (objDatabaseHelper.UpdateChargeLog_Supplies2_sx(orderId, tid, msg, Request.UserHostAddress, CallbackLog, Year, ref ChargeType))
                                {
                                    if (objSMSHepler.SendMsg_SL(mobile, msg))
                                    {

                                        //m2 = m2.IndexOf("aid=") > 0 ? m2 : (m2.IndexOf("?") > 0 ? m2 + "&aid=" + m1 : m2 + "?aid=" + m1 + "&a=" + adminID);
                                        m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=19&a=" + adminID + "&aid=" + aid + (ChargeType == "Twm" ? "&twm=1" : "");
                        ...
                        {
                            if (objDatabaseHelper.UpdateChargeStatus_Supplies2_sx(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                            {
                        ...
                        {
                            objDatabaseHelper.UpdateChargeStatus_Supplies2_sx(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
                    ...
                    {
                        //已經付費成功。
                        m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=17&a=33&aid=" + aid + (dtCharge.Rows[0]["ChargeType"].ToString() == "Twm" ? "&twm=1" : "");
                        Response.Redirect(m2, true);
                    }
                    else
                    {
