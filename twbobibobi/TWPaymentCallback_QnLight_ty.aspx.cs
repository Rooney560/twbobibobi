...
namespace twbobibobi
{
    ...
    /// 
    /// </summary>
    public partial class TWPaymentCallback_QnLight_ty : BasePage
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                try
                {
                    ...
                    dtCharge = objLightDAC.GetChargeLog_QnLight_ty(orderId, Year);
                    ...
                    if (dtCharge.Rows.Count > 0)
                    {
                        ...
                        int qnlighttype = objLightDAC.GetQnLightType_ty(aid, Year);

                        string rebackURL = "https://bobibobi.tw/Temples/templeService_qnlight_ty.aspx";
                        ...
                        if (status == 0)
                        {
                            ...
                            if (result[0] == "1" || result[0] == "2")
                            {
                                if (result.Length > 1)
                                {
                                    ...
                                    //更新普渡資料表並取得訂單編號
                                    objLightDAC.UpdateQnLight_ty_Info(aid, qnlighttype, Year, ref msg, ref qnlightlist, ref QnLightlist);
                                    ////更新購買表內購買人狀態為已付款(Status=2)
                                    objLightDAC.Updateapplicantinfo_QnLight_ty(aid, cost, 2, Year);

                                    SMSHepler objSMSHepler = new SMSHepler();
                                    string ChargeType = string.Empty;
                                    int uStatus = 0;
                                    //更新流水付費表資訊(付費成功)
                                    if (objLightDAC.UpdateChargeLog_QnLight_ty(orderId, tid, msg, Request.UserHostAddress, CallbackLog, Year, ref ChargeType, ref uStatus))
                                    {
                                        if (invStatus)
                                        {
                                            try
                                            {
                                                ...
                                                DataTable dtApplicantInfo = objALightDAC.GetAPPCharge_ty_QnLight(aid, Year);
                                                ...
                                                if (dtApplicantInfo.Rows.Count > 0)
                                                {
                                                    ...
                                                    NumString += string.Join(",", QnLightlist);

                                                    foreach (DataRow row in dtApplicantInfo.Rows)
                                                    {
                                                        string QnLightType = row["QnLightType"].ToString();
                                                        string QnLightString = row["QnLightString"].ToString();
                                                        string description = $"線上服務費({templeName}-{QnLightString})";
                                                        ...
                                                        // 取單價
                                                        int price = InvoiceHelper.GetUnitPrice(adminId, int.Parse(kind), QnLightType);
                                                        ...
                                                        {
                                                            string errormsg = $"❌ 金額不符，OrderID={row["OrderID"]}, Kind={kind}, Service={QnLightString}, " +
                                                        ...
                                                        {
                                                            Description = "線上服務費",
                                                            ProductName = $"{templeName}-{QnLightString}",
                                                            Quantity = quantity,
                                                    ...
                                                    if (rs.Success)
                                                    {
                                                        ...
                                                        if (objLightDAC.UpdateInvoiceDetail(aid, adminId, 25, invoiceNo, rawJson, "1", Year))
                                                        {
                                                            if (InvoiceEmailSender.Send(rs, Emailitems, buyerEmail, buyerName, invoiceRow["BuyerIdentifier"]?.ToString() ?? "0000000000", NumString, cost, dtNow, YearROC, Month, Date, Time))
                                                            {
                                                                ...
                                                                {
                                                                    // ❌ 傳送簡訊失敗，記錄錯誤
                                                                    SaveErrorLog("TWPaymentCallback_QnLight_ty" + $"傳送簡訊失敗");
                                                            ...
                                                            {
                                                                // ❌ 寄送EMAIL失敗，記錄錯誤
                                                                SaveErrorLog("TWPaymentCallback_QnLight_ty" + $"寄送EMAIL失敗");
                                                            }
                                                        }
                                                        else
                                                        {
                                                            // ❌ 發票失敗，記錄錯誤
                                                            SaveErrorLog("TWPaymentCallback_QnLight_ty" + $"更新發票失敗 AdminID: {adminId}" + new Exception(rs.ErrorMessage));
                                                        }
                                                    }
                                                    else
                                                    {
                                                        // ❌ 發票開立失敗 → 只寫 Log，不影響訂單流程
                                                        SaveErrorLog("TWPaymentCallback_QnLight_ty 開立發票失敗 AdminID="
                                                ...
                                                {
                                                    // ❌ 個別筆數處理例外錯誤
                                                    SaveErrorLog("TWPaymentCallback_QnLight_ty" + $"開立發票處理例外, 取得訂單錯誤。");
                                                }
                                            ...
                                            {
                                                // ❌ 個別筆數處理例外錯誤
                                                SaveErrorLog("TWPaymentCallback_QnLight_ty" + $"開立發票處理例外 ex: " + ex.InnerException.Message);
                                            }
                                        }
                                    ...
                                    {
                                        // ❌ 更新流水付費表資訊，記錄錯誤
                                        SaveErrorLog("TWPaymentCallback_QnLight_ty" + $"更新流水付費表資訊失敗");
                            ...
                            {
                                if (objLightDAC.UpdateChargeStatus_QnLight_ty(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                                {
                                    // ❌ 此用戶已退款，記錄錯誤
                                    SaveErrorLog("TWPaymentCallback_QnLight_ty" + $"此用戶已退款");
                                    Response.Write("<script>alert('此用戶已退款。');window.location.href='" + rebackURL + "'</script>");
                                }
                            }
                            else
                            {
                                objLightDAC.UpdateChargeStatus_QnLight_ty(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
                                ...
                                {
                                    // ❌ 付款失敗，記錄錯誤
                                    SaveErrorLog("TWPaymentCallback_QnLight_ty" + $"付款失敗，錯誤代碼：" + result[0]);
                        ...
                        {
                            //已經付費成功。
                            m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=" + kind + "&a=14&aid=" + aid + (dtCharge.Rows[0]["ChargeType"].ToString() == "Twm" ? "&twm=1" : "");
                            Response.Redirect(m2, true);
                        }
                        else if (status == -2)
                        {
