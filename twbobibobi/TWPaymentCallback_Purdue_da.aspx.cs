...
namespace Temple
{
    public partial class TWPaymentCallback_Purdue_da : BasePage
    {
                ...
                try
                {
                    ...
                    if (dtCharge.Rows.Count > 0)
                    {
                        ...
                        if (status == 0)
                        {
                            ...
                            string rebackURL = "https://bobibobi.tw/Temples/templeService_purdue_da.aspx";
                            ...
                            if (result[0] == "1" || result[0] == "2")
                            {
                                if (result.Length > 1)
                                {
                                    ...
                                    //取得申請人資料表
                                    //DataTable dtapplicantinfo = objDatabaseHelper.Getapplicantinfo_Purdue_da(aid, adminID, Year);
                                    ////更新購買表內購買人狀態為已付款(Status=2)
                                    //int cost = dtapplicantinfo.Rows.Count > 0 ? int.Parse(dtapplicantinfo.Rows[0]["Cost"].ToString()) : 0;
                                    objDatabaseHelper.Updateapplicantinfo_Purdue_da(aid, cost, 2, Year);
                            ...
                            {
                                if (objDatabaseHelper.UpdateChargeStatus_Purdue_da(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                                {
                            ...
                            {
                                objDatabaseHelper.UpdateChargeStatus_Purdue_da(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
                        ...
                        {
                            //已經付費成功。
                            m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=2&a=3&aid=" + aid + (dtCharge.Rows[0]["ChargeType"].ToString() == "Twm" ? "&twm=1" : "");
                            Response.Redirect(m2, true);
                        }
                        else
                        {
                    ...
                    //DataTable dtCharge = objDatabaseHelper.GetChargeLog_Purdue_da(orderId);
                    ...
                    //            int[] typelist = objDatabaseHelper.GetPurdueType_da(aid, adminID);

                    //            //更新普渡資料表並取得訂單編號
                    //            objDatabaseHelper.UpdatePurdue_da_Info(aid, typelist, ref purduelist);
                    //            //取得申請人資料表
                    //            DataTable dtapplicantinfo = objDatabaseHelper.Getapplicantinfo_Purdue_da(aid, adminID);
                    //            //更新購買表內購買人狀態為已付款(Status=2)
                    //            objDatabaseHelper.Updateapplicantinfo_Purdue_da(aid);
                    ...
                    //            //更新流水付費表資訊(付費成功)
                    //            if (objDatabaseHelper.UpdateChargeLog_Purdue_da(orderId, tid, msg, Request.UserHostAddress, CallbackLog))
                    //            {
                    //                if (objSMSHepler.SendMsg_SL(mobile, msg))
                    //                {
                    ...
                    //                else
                    //                {
                    //                    Response.Write("<script>alert('傳送簡訊失敗。請聯繫管理員。客服電話：04-23582760。');window.location.href='https://bobibobi.tw/Temples/templeService_purdue.aspx?a=3'</script>");
                    //                }
                    //            }
                    //            else
                    //            {
                    //                Response.Write("<script>alert('付款過程失敗。請聯繫管理員。客服電話：04-23582760。');window.location.href='https://bobibobi.tw/Temples/templeService_purdue.aspx?a=3'</script>");
                    //            }
                    //        }
                    //        else if (result[0] == "4")
                    //        {
                    //            if (objDatabaseHelper.UpdateChargeStatus_Purdue_da(orderId, -2))
                    //            {
                    //                Response.Write("<script>alert('此用戶已退款。');window.location.href='https://bobibobi.tw/Temples/templeService_purdue.aspx?a=3'</script>");
                    //            }
                    //        }
                    //        else
                    //        {
                    //            objDatabaseHelper.UpdateChargeErrLog_Purdue_da(orderId, "", Request.UserHostAddress, CallbackLog);
                    ...
                    //            else
                    //            {
                    //                Response.Write("<script>alert('付款失敗，錯誤代碼：" + result[0] + "。客服電話：04-23582760。');window.location.href='https://bobibobi.tw/Temples/templeService_purdue.aspx?a=3'</script>");
                    //            }
                    //        }
                    //    }
                    ...
                    //Response.Write("<script>alert('取得付款資料失敗，錯誤代碼：" + resp + "。客服電話：04-23582760。');window.location.href='https://bobibobi.tw/Temples/templeService_purdue.aspx?a=3'</script>");
                }
