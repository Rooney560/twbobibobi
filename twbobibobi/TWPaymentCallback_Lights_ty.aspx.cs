...
namespace Temple
{
    public partial class TWPaymentCallback_Lights_ty : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                ...
                dtCharge = objDatabaseHelper.GetChargeLog_Lights_ty(orderId, Year);
                ...
                if (dtCharge.Rows.Count > 0)
                {
                    ...
                    string rebackURL = "https://bobibobi.tw/Temples/templeService_lights_ty.aspx";

                    if (lightstype == 21)
                    {
                        //孝親祈福燈
                        rebackURL = "https://bobibobi.tw/Temples/templeService_lights_ty_mom.aspx";
                    }
                    ...
                    if (status == 0)
                    {
                            ...
                            if (result.Length > 1)
                            {
                                ...
                                if (objDatabaseHelper.UpdateChargeLog_Lights_ty(orderId, tid, msg, Request.UserHostAddress, CallbackLog, Year, ref ChargeType))
                                {
                                    if (objSMSHepler.SendMsg_SL(mobile, msg))
                                    {
                                        ...
                                        m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=1&a=" + adminID + "&aid=" + aid + (ChargeType == "Twm" ? "&twm=1" : "") + (lightstype == 21 ? "&type=2" : "&type=1");
                        ...
                        {
                            if (objDatabaseHelper.UpdateChargeStatus_Lights_ty(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                            {
                        ...
                        {
                            objDatabaseHelper.UpdateChargeStatus_Lights_ty(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
                    ...
                    {
                        //已經付費成功。
                        m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=1&a=14&aid=" + aid + (lightstype == 21 ? "&type=2" : "&type=1") + (dtCharge.Rows[0]["ChargeType"].ToString() == "Twm" ? "&twm=1" : "");
                        Response.Redirect(m2, true);
                    }
                    else
                    {
                ...
                //    dtCharge = objDatabaseHelper.GetChargeLog_Lights_ty_mom(orderId, Year);
                //    if (dtCharge.Rows.Count > 0)
                //    {
                ...
                //            if (result[0] == "1" || result[0] == "2")
                //            {
                //                bool update_charge_log = false;
                //                bool update_info_log = false;
                //                bool update_applicant_log = false;
                //                string mobile = result[5];
                //                int adminID = 14;
                //                int cost = 0;
                ...
                //                //更新購買表內購買人狀態為已付款(Status=2)
                //                cost = int.Parse(dtapplicantinfo.Rows[0]["Cost"].ToString());
                //                update_applicant_log = objDatabaseHelper.Updateapplicantinfo_Lights_ty_mom(aid, cost, 2, Year);

                //                if (cost > 0 && dtapplicantinfo.Rows.Count > 0 && update_applicant_log && update_info_log)
                //                {
                //                    msg = "感謝購買,已成功付款" + dtapplicantinfo.Rows[0]["Cost"].ToString() + "元,您的訂單編號 ";
                ...
                //                    update_charge_log = objDatabaseHelper.UpdateChargeLog_Lights_ty_mom(orderId, tid, msg, Request.UserHostAddress, CallbackLog, Year, ref ChargeType);

                //                    //更新流水付費表資訊(付費成功)
                //                    if (update_charge_log)
                //                    {
                //                        if (objSMSHepler.SendMsg_SL(mobile, msg))
                //                        {

                //                            //m2 = m2.IndexOf("aid=") > 0 ? m2 : (m2.IndexOf("?") > 0 ? m2 + "&aid=" + m1 : m2 + "?aid=" + m1 + "&a=" + adminID);
                //                            m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=1&a=" + adminID + "&aid=" + aid + (ChargeType == "Twm" ? "&twm=1" : "") + (type == 2 ? "&type=2" : "&type=1");
                //                            Response.Redirect(m2, true);
                //                        }
                //                        else
                //                        {
