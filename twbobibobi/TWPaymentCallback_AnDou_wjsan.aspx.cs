    ...
    public partial class TWPaymentCallback_AnDou_wjsan : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                ...
                DataTable dtCharge = objDatabaseHelper.GetChargeLog_AnDou_wjsan(orderId, Year);
                ...
                if (dtCharge.Rows.Count > 0)
                {
                    ...
                    if (status == 0)
                    {
                        int type = objDatabaseHelper.GetAnDouType_wjsan(aid, Year);
                        ...
                        if (result[0] == "1" || result[0] == "2")
                        {
                            if (result.Length > 1)
                            {
                                ...
                                //更新普渡資料表並取得訂單編號
                                objDatabaseHelper.UpdateAnDou_wjsan_Info(aid, type, Year, ref msg, ref andoulist, ref AnDoulist);
                                //取得申請人資料表
                                //DataTable dtapplicantinfo = objDatabaseHelper.Getapplicantinfo_AnDou_wjsan(aid, adminID, Year);
                                ////更新購買表內購買人狀態為已付款(Status=2)
                                //int cost = dtapplicantinfo.Rows.Count > 0 ? int.Parse(dtapplicantinfo.Rows[0]["Cost"].ToString()) : 0;
                                objDatabaseHelper.Updateapplicantinfo_AnDou_wjsan(aid, cost, 2, Year);
                                ...
                                //更新流水付費表資訊(付費成功)
                                if (objDatabaseHelper.UpdateChargeLog_AnDou_wjsan(orderId, tid, msg, Request.UserHostAddress, CallbackLog, Year, ref ChargeType))
                                {
                                    if (objSMSHepler.SendMsg_SL(mobile, msg))
                                    {

                                        //m2 = m2.IndexOf("aid=") > 0 ? m2 : (m2.IndexOf("?") > 0 ? m2 + "&aid=" + m1 : m2 + "?aid=" + m1 + "&a=" + adminID);
                                        m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=20&a=" + adminID + "&ad=2&aid=" + aid +
                        ...
                        {
                            if (objDatabaseHelper.UpdateChargeStatus_AnDou_wjsan(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                            {
                        ...
                        {
                            objDatabaseHelper.UpdateChargeStatus_AnDou_wjsan(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
