    ...
    public partial class TWPaymentCallback_Lights_Hs : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                ...
                DataTable dtCharge = objDatabaseHelper.GetChargeLog_Lights_Hs(orderId, Year);
                ...
                if (dtCharge.Rows.Count > 0)
                {
                    ...
                    if (status == 0)
                    {
                        int type = objDatabaseHelper.GetLightsType_Hs(aid, Year);
                        ...
                        if (result[0] == "1" || result[0] == "2")
                        {
                            if (result.Length > 1)
                            {
                                ...
                                //更新普渡資料表並取得訂單編號
                                objDatabaseHelper.UpdateLights_Hs_Info(aid, type, Year, ref msg, ref lightslist, ref Lightslist);
                                //取得申請人資料表
                                //DataTable dtapplicantinfo = objDatabaseHelper.Getapplicantinfo_Lights_Hs(aid, adminID, Year);
                                ////更新購買表內購買人狀態為已付款(Status=2)
                                //int cost = dtapplicantinfo.Rows.Count > 0 ? int.Parse(dtapplicantinfo.Rows[0]["Cost"].ToString()) : 0;
                                objDatabaseHelper.Updateapplicantinfo_Lights_Hs(aid, cost, 2, Year);
                                ...
                                //更新流水付費表資訊(付費成功)
                                if (objDatabaseHelper.UpdateChargeLog_Lights_Hs(orderId, tid, msg, Request.UserHostAddress, CallbackLog, Year, ref ChargeType))
                                {
                        ...
                        {
                            if (objDatabaseHelper.UpdateChargeStatus_Lights_Hs(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                            {
                        ...
                        {
                            objDatabaseHelper.UpdateChargeStatus_Lights_Hs(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
                    ...
                    {
                        //已經付費成功。
                        m2 = "https://bobibobi.tw/Temples/templeComplete.aspx?kind=1&a=17&aid=" + aid + (dtCharge.Rows[0]["ChargeType"].ToString() == "Twm" ? "&twm=1" : "");
                        Response.Redirect(m2, true);
                    }
                    else
                    {
