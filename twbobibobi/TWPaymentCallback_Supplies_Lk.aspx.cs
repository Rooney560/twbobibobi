...
{
    public partial class TWPaymentCallback_Supplies_Lk : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                try
                {
                    ...
                    DataTable dtCharge = objLightDAC.GetChargeLog_Supplies_Lk(orderId, Year);
                    ...
                    if (dtCharge.Rows.Count > 0)
                    {
                        ...
                        if (status == 0)
                        {
                            string rebackURL = "https://bobibobi.tw/Temples/templeService_supplies_Lk.aspx";
                            ...
                            if (result[0] == "1" || result[0] == "2")
                            {
                                if (result.Length > 1)
                                {
                                    ...
                                    objLightDAC.Updateapplicantinfo_Supplies_Lk(aid, cost, 2, Year);

                                    SMSHepler objSMSHepler = new SMSHepler();
                                    string ChargeType = string.Empty;
                                    int uStatus = 0;
                                    //更新流水付費表資訊(付費成功)
                                    if (objLightDAC.UpdateChargeLog_Supplies_Lk(orderId, tid, msg, Request.UserHostAddress, CallbackLog, Year, ref ChargeType, ref uStatus))
                                    {
                                        if (invStatus)
                                        {
                                            try
                                            {
                                                ...
                                                DataTable dtApplicantInfo = objALightDAC.GetAPPCharge_Lk_Supplies(aid, Year);
                                                ...
                                                if (dtApplicantInfo.Rows.Count > 0)
                                                {
                                                    ...
                                                    if (rs.Success)
                                                    {
                                                        ...
                                                        if (objLightDAC.UpdateInvoiceDetail(aid, adminId, 16, invoiceNo, rawJson, "1", Year))
                                                        {
                                                            if (InvoiceEmailSender.Send(rs, Emailitems, buyerEmail, buyerName, invoiceRow["BuyerIdentifier"]?.ToString() ?? "0000000000", NumString, cost, dtNow, YearROC, Month, Date, Time))
                                                            {
                                                                ...
                                                                {
                                                                    // ❌ 傳送簡訊失敗，記錄錯誤
                                                                    SaveErrorLog("TWPaymentCallback_Supplies_Lk" + $"傳送簡訊失敗");
                                                            ...
                                                            {
                                                                // ❌ 寄送EMAIL失敗，記錄錯誤
                                                                SaveErrorLog("TWPaymentCallback_Supplies_Lk" + $"寄送EMAIL失敗");
                                                            }
                                                        }
                                                        else
                                                        {
                                                            // ❌ 發票失敗，記錄錯誤
                                                            SaveErrorLog("TWPaymentCallback_Supplies_Lk" + $"更新發票失敗 AdminID: {adminId}" + new Exception(rs.ErrorMessage));
                                                        }
                                                    }
                                                    else
                                                    {
                                                        // ❌ 發票開立失敗 → 只寫 Log，不影響訂單流程
                                                        SaveErrorLog("TWPaymentCallback_Supplies_Lk 開立發票失敗 AdminID="
                                                ...
                                                {
                                                    // ❌ 個別筆數處理例外錯誤
                                                    SaveErrorLog("TWPaymentCallback_Supplies_Lk" + $"開立發票處理例外, 取得訂單錯誤。");
                                                }
                                            ...
                                            {
                                                // ❌ 個別筆數處理例外錯誤
                                                SaveErrorLog("TWPaymentCallback_Supplies_Lk" + $"開立發票處理例外 ex: " + ex.InnerException.Message);
                                            }
                                    ...
                                    {
                                        // ❌ 更新流水付費表資訊失敗，記錄錯誤
                                        SaveErrorLog("TWPaymentCallback_Supplies_Lk" + $"更新流水付費表資訊失敗");
                            ...
                            {
                                if (objLightDAC.UpdateChargeStatus_Supplies_Lk(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                                {
                                    // ❌ 此用戶已退款，記錄錯誤
                                    SaveErrorLog("TWPaymentCallback_Supplies_Lk" + $"此用戶已退款");
                                    Response.Write("<script>alert('此用戶已退款。');window.location.href='" + rebackURL + "'</script>");
                                }
                            }
                            else
                            {
                                objLightDAC.UpdateChargeStatus_Supplies_Lk(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
                                ...
                                {
                                    // ❌ 付款失敗，記錄錯誤
                                    SaveErrorLog("TWPaymentCallback_Supplies_Lk" + $"付款失敗，錯誤代碼：" + result[0]);
