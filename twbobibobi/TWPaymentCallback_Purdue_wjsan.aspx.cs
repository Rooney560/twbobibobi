...
{
    public partial class TWPaymentCallback_Purdue_wjsan : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                try
                {
                    ...
                    DataTable dtCharge = objLightDAC.GetChargeLog_Purdue_wjsan(orderId, Year);
                    ...
                    if (dtCharge.Rows.Count > 0)
                    {
                        ...
                        if (status == 0)
                        {
                            int type = objLightDAC.GetPurdueType_wjsan(aid, Year);

                            string rebackURL = "https://bobibobi.tw/Temples/templeService_purdue_wjsan.aspx";
                            ...
                            if (result[0] == "1" || result[0] == "2")
                            {
                                if (result.Length > 1)
                                {
                                    ...
                                    //更新普渡資料表並取得訂單編號
                                    objLightDAC.UpdatePurdue_wjsan_Info(aid, Year, ref msg, ref purduelist, ref Purduelist);
                                    ////更新購買表內購買人狀態為已付款(Status=2)
                                    objLightDAC.Updateapplicantinfo_Purdue_wjsan(aid, cost, 2, Year);

                                    SMSHepler objSMSHepler = new SMSHepler();
                                    string ChargeType = string.Empty;
                                    int uStatus = 0;
                                    //更新流水付費表資訊(付費成功)
                                    if (objLightDAC.UpdateChargeLog_Purdue_wjsan(orderId, tid, msg, Request.UserHostAddress, CallbackLog, Year, ref ChargeType, ref uStatus))
                                    {
                                        if (invStatus)
                                        {
                                            try
                                            {
                                                ...
                                                DataTable dtApplicantInfo = objALightDAC.GetAPPCharge_wjsan_Purdue(aid, Year);
                                                    ...
                                                    if (rs.Success)
                                                    {
                                                        ...
                                                        if (objLightDAC.UpdateInvoiceDetail(aid, adminId, 2, invoiceNo, rawJson, "1", Year))
                                                        {
                                                            if (InvoiceEmailSender.Send(rs, Emailitems, buyerEmail, buyerName, invoiceRow["BuyerIdentifier"]?.ToString() ?? "0000000000", NumString, cost, dtNow, YearROC, Month, Date, Time))
                                                            {
                                                                ...
                                                                {
                                                                    // ❌ 傳送簡訊失敗，記錄錯誤
                                                                    SaveErrorLog("TWPaymentCallback_Purdue_wjsan" + $"傳送簡訊失敗");
                                                            ...
                                                            {
                                                                // ❌ 寄送EMAIL失敗，記錄錯誤
                                                                SaveErrorLog("TWPaymentCallback_Purdue_wjsan" + $"寄送EMAIL失敗");
                                                            }
                                                        }
                                                        else
                                                        {
                                                            // ❌ 發票失敗，記錄錯誤
                                                            SaveErrorLog("TWPaymentCallback_Purdue_wjsan" + $"更新發票失敗");
                                                        }
                                                    }
                                                    else
                                                    {
                                                        // ❌ 發票開立失敗 → 只寫 Log，不影響訂單流程
                                                        SaveErrorLog("TWPaymentCallback_Purdue_wjsan 開立發票失敗 AdminID="
                                                ...
                                                {
                                                    // ❌ 個別筆數處理例外錯誤
                                                    SaveErrorLog("TWPaymentCallback_Purdue_wjsan" + $"開立發票處理例外, 取得訂單錯誤。");
                                                }
                                            ...
                                            {
                                                // ❌ 個別筆數處理例外錯誤
                                                SaveErrorLog("TWPaymentCallback_Purdue_wjsan" + $"開立發票處理例外 ex: " + ex.Message);
                                            }
                                    ...
                                    {
                                        // ❌ 更新流水付費表資訊失敗，記錄錯誤
                                        SaveErrorLog("TWPaymentCallback_Purdue_wjsan" + $"更新流水付費表資訊失敗");
                            ...
                            {
                                if (objLightDAC.UpdateChargeStatus_Purdue_wjsan(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                                {
                                    // ❌ 此用戶已退款，記錄錯誤
                                    SaveErrorLog("TWPaymentCallback_Purdue_wjsan" + $"此用戶已退款");
                                    Response.Write("<script>alert('此用戶已退款。');window.location.href='" + rebackURL + "'</script>");
                                }
                            }
                            else
                            {
                                objLightDAC.UpdateChargeStatus_Purdue_wjsan(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
                                ...
                                {
                                    // ❌ 付款失敗，記錄錯誤
                                    SaveErrorLog("TWPaymentCallback_Purdue_wjsan" + $"付款失敗，錯誤代碼：" + result[0]);
