    ...
    public partial class TWPaymentCallback_Supplies_ty : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request["uid"] != null && Request["oid"] != null && Request["tid"] != null && Request["mac"] != null)
            {
                ...
                int status = 999;
                int adminID = objDatabaseHelper.GetAdminID_Supplies_ty(aid, Year);
                ...
                if (dtCharge.Rows.Count > 0)
                {
                    ...
                    if (status == 0)
                    {
                        ...
                        {
                            if (objDatabaseHelper.UpdateChargeStatus_Supplies_ty(orderId, -2, Request.UserHostAddress, CallbackLog, Year))
                            {
                        ...
                        {
                            objDatabaseHelper.UpdateChargeStatus_Supplies_ty(orderId, -1, Request.UserHostAddress, CallbackLog, Year);
                            //objDatabaseHelper.UpdateChargeErrLog_Supplies_ty(orderId, "", Request.UserHostAddress, CallbackLog, Year);
